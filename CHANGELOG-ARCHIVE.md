# Released-version changelog archive

The repository-root `CHANGELOG.md` carries the `## Unreleased` section — the
entries for work that has not shipped yet, which is what a reviewer and the
release job both read. Everything that has already been published lives here,
newest first, continuing from the last released heading below.

The seam is the release boundary, not a line count: `CHANGELOG.md` crossed the
10,000-line tracked-file limit (issue #780) because the unreleased section alone
had grown past it, and splitting anywhere inside that section would have cut a
single release's notes in half. `.github/workflows/publish.yml` reads only the
top `## ` heading of `CHANGELOG.md`, so the release gate is unaffected.
