...
- **Event histories: the latent covariance is learned, its rank is grown by
  ...
  Two things the discretisation had to stop inventing: the compensator is
  now integrated by a closed rule (`gam_math::special::gauss_lobatto`, new)
  so an event time carries its own exposure — with the open rule the
  instant whose intensity the likelihood reads had none, its latent
  coordinate was held only by the prior, and the evidence gained `a²/2` per
  event without bound — and the covariance score is standardised by the
  full information, including the process's own diagonal `¼ Σ v⁴ c̄`, so
  the rate it names is a matched filter rather than the fastest kernel the
  mesh can represent.
