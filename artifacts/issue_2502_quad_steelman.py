"""Second-order curved atoms: does the win GROW with curve order?

Atom contribution a·(U + t·V + t²·W), 3P decoder params per atom, same TopK
ReLU amplitude head and tanh coordinate head, same Gao-et-al recipe as
curved_steel.py. Parity: quad K vs curved 1.5K vs flat 3K.

Eval-time coordinate: the gain ⟨R,d(t)⟩²/‖d(t)‖² with quadratic d(t) has a
quintic stationarity, so t is taken as the best of a 33-point Chebyshev grid
followed by one projected Newton refine — an experiment-script compromise the
linear-curve arms do not need (their optimum is closed-form).
"""

import json
import os
import sys

import numpy as np

V2 = os.path.expanduser("~/i2502v2")


def ev(x, r):
    return 1.0 - ((x - r) ** 2).sum() / (x ** 2).sum()


def main() -> int:
    import torch

    K = int(sys.argv[1])
    seed = int(sys.argv[2])
    epochs = int(sys.argv[3]) if len(sys.argv) > 3 else 300
    k_act = int(sys.argv[4]) if len(sys.argv) > 4 else 8
    k_aux = 512
    aux_weight = 1.0 / 32.0

    Xtr = np.load(f"{V2}/train_chart.npy")
    Xte = np.load(f"{V2}/test_chart.npy")
    lift = np.load(f"{V2}/lift.npy")
    c0 = np.load(f"{V2}/c0.npy")
    amb = np.load(f"{V2}/test_ambient.npy").astype(np.float64)
    P = Xtr.shape[1]
    dev = "cuda:0"
    t_data = torch.tensor(Xtr, dtype=torch.float32, device=dev)

    g = torch.Generator(device=dev).manual_seed(seed)
    U = torch.randn(K, P, generator=g, device=dev)
    U /= U.norm(dim=1, keepdim=True)
    U = torch.nn.Parameter(U)
    V = torch.randn(K, P, generator=g, device=dev)
    V.mul_(0.1 / V.norm(dim=1, keepdim=True))
    V = torch.nn.Parameter(V)
    W = torch.randn(K, P, generator=g, device=dev)
    W.mul_(0.05 / W.norm(dim=1, keepdim=True))
    W = torch.nn.Parameter(W)
    W_enc = torch.nn.Parameter(U.detach().t().clone())
    b_enc = torch.nn.Parameter(torch.zeros(K, device=dev))
    W_t = torch.nn.Parameter(0.01 * torch.randn(P, K, generator=g, device=dev))
    b_t = torch.nn.Parameter(torch.zeros(K, device=dev))
    b_pre = torch.nn.Parameter(t_data.mean(0).clone())
    opt = torch.optim.Adam([U, V, W, W_enc, b_enc, W_t, b_t, b_pre], lr=1e-3)

    n = len(t_data)
    bs = 4096
    steps_per_epoch = (n + bs - 1) // bs
    total = epochs * steps_per_epoch
    warm = max(1, total // 50)
    sched = torch.optim.lr_scheduler.LambdaLR(
        opt,
        lambda s: (s + 1) / warm if s < warm
        else 0.5 * (1.0 + np.cos(np.pi * (s - warm) / max(1, total - warm))),
    )
    last_fired = torch.zeros(K, dtype=torch.long, device=dev)

    for _ep in range(epochs):
        perm = torch.randperm(n, generator=g, device=dev)
        for s in range(0, n, bs):
            xb = t_data[perm[s:s + bs]]
            xc = xb - b_pre
            pre = xc @ W_enc + b_enc
            tcoord = torch.tanh(xc @ W_t + b_t)
            val, idx = torch.topk(pre, k_act, dim=1)
            z = torch.zeros_like(pre).scatter_(1, idx, torch.relu(val))
            zt = z * tcoord
            zt2 = zt * tcoord
            recon = z @ U + zt @ V + zt2 @ W + b_pre
            residual = xb - recon
            loss = (residual ** 2).mean()

            with torch.no_grad():
                fired = torch.zeros(K, dtype=torch.bool, device=dev)
                fired.scatter_(0, idx.reshape(-1), True)
                last_fired += 1
                last_fired[fired] = 0
                dead_mask = last_fired > 8 * steps_per_epoch
            if dead_mask.any():
                dead_pre = pre.masked_fill(~dead_mask.unsqueeze(0), float("-inf"))
                kk = int(min(k_aux, int(dead_mask.sum().item())))
                if kk > 0:
                    aval, aidx = torch.topk(dead_pre, kk, dim=1)
                    az = torch.zeros_like(pre).scatter_(1, aidx, torch.relu(aval))
                    azt = az * tcoord
                    aux_recon = az @ U + azt @ V + (azt * tcoord) @ W
                    loss = loss + aux_weight * ((residual.detach() - aux_recon) ** 2).mean()

            opt.zero_grad()
            loss.backward()
            opt.step()
            sched.step()
            with torch.no_grad():
                U.data /= U.data.norm(dim=1, keepdim=True).clamp_min(1e-8)

    with torch.no_grad():
        Ud, Vd, Wd = U.detach().float(), V.detach().float(), W.detach().float()
        bp = b_pre.detach().float()
        x = torch.tensor(Xte, dtype=torch.float32, device=dev)
        xc = x - bp
        N = x.shape[0]
        R = xc.clone()
        picks = torch.zeros(N, k_act, dtype=torch.long, device=dev)
        tpick = torch.zeros(N, k_act, device=dev)
        taken = torch.zeros(N, K, dtype=torch.bool, device=dev)
        grid = torch.cos(torch.pi * (torch.arange(33, device=dev) + 0.5) / 33)

        def decode(p, t):
            return Ud[p] + t.unsqueeze(1) * Vd[p] + (t ** 2).unsqueeze(1) * Wd[p]

        for s in range(k_act):
            # gain per atom at each grid t, batched: candidates (33, N, K)
            best_gain = torch.full((N, K), float("-inf"), device=dev)
            best_t = torch.zeros(N, K, device=dev)
            rU, rV, rW = R @ Ud.T, R @ Vd.T, R @ Wd.T
            uu = (Ud * Ud).sum(1)
            uv = (Ud * Vd).sum(1)
            uw = (Ud * Wd).sum(1)
            vv = (Vd * Vd).sum(1)
            vw = (Vd * Wd).sum(1)
            ww = (Wd * Wd).sum(1)
            for tg in grid:
                num = rU + tg * rV + tg * tg * rW
                den = (uu + 2 * tg * uv + tg * tg * (vv + 2 * uw)
                       + 2 * tg ** 3 * vw + tg ** 4 * ww).clamp_min(1e-12)
                gain = torch.where(num > 0, num ** 2 / den.unsqueeze(0),
                                   torch.zeros_like(num))
                better = gain > best_gain
                best_gain = torch.where(better, gain, best_gain)
                best_t = torch.where(better, torch.full_like(best_t, float(tg)), best_t)
            best_gain = best_gain.masked_fill(taken, float("-inf"))
            p = best_gain.argmax(1)
            picks[:, s] = p
            taken.scatter_(1, p.unsqueeze(1), True)
            tp = best_t.gather(1, p.unsqueeze(1)).squeeze(1)
            tpick[:, s] = tp
            d = decode(p, tp)
            dn = (d * d).sum(1).clamp_min(1e-30)
            a = ((R * d).sum(1) / dn).clamp_min(0.0)
            R = R - a.unsqueeze(1) * d

        A = (Ud[picks] + tpick.unsqueeze(-1) * Vd[picks]
             + (tpick ** 2).unsqueeze(-1) * Wd[picks]).transpose(1, 2)
        sol = torch.linalg.lstsq(A, xc.unsqueeze(-1)).solution
        rec_chart = ((A @ sol).squeeze(-1) + bp).cpu().numpy().astype(np.float64)
        alive = int(torch.unique(picks).numel())

    rec_amb = rec_chart @ lift + c0
    out = {
        "arm": "quad_steelman", "K": K, "seed": seed, "L0": k_act, "epochs": epochs,
        "decoder_params": int(K * 3 * P),
        "alive_on_test_routing": alive, "dead": K - alive, "dead_frac": (K - alive) / K,
        "W_norm_median": float(Wd.norm(dim=1).median()),
        "V_norm_median": float(Vd.norm(dim=1).median()),
        "chart_ev_omp_ls": float(ev(Xte, rec_chart)),
        "ambient_ev": float(ev(amb, rec_amb)),
    }
    print(json.dumps(out, indent=1), flush=True)
    np.savez(f"{V2}/quad_k{K}_s{seed}_l{k_act}.npz",
             U=Ud.cpu().numpy(), V=Vd.cpu().numpy(), W=Wd.cpu().numpy(),
             b_pre=bp.cpu().numpy(), k_act=k_act)
    return 0


if __name__ == "__main__":
    sys.exit(main())
