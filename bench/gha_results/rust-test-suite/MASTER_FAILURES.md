...
- Python test failures: **NOT MEASURED — at least 7** (LOWER BOUND, not a count: Python API tests (job `cancelled`); Python populations, slow + torch (job `cancelled`) did not run to completion, so the tests they never reached are unmeasured, not passing)
...
- `crates/gam-sae/src/manifold/construction_tests.rs:2094:26` — [E0596] cannot borrow `*term` as mutable, as it is behind a `&` reference
- `?` — could not compile `gam-sae` (lib test) due to 1 previous error
...
_Lower bound: 7 recorded before the run stopped. Unmeasured: Python API tests (job `cancelled`); Python populations, slow + torch (job `cancelled`)._
