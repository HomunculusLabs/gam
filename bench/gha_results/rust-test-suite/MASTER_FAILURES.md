...
- Python test failures: **NOT MEASURED — at least 155** (LOWER BOUND, not a count: Python populations, slow + torch (job `cancelled`) did not run to completion, so the tests they never reached are unmeasured, not passing)
...
- **FAIL** `gam-models::probe_2612_separated_confidence` :: `zz_probe_2612_separated_multinomial_is_under_confident`
...
- **FAIL** `gam::regressions` :: `families::multinomial_predict_requires_response_column::multinomial_predict_reports_a_genuinely_missing_feature_by_name`
...
- **FAIL** `gam::regressions` :: `families::multinomial_predict_requires_response_column::multinomial_predicts_on_label_free_and_reordered_frames`
...
_Lower bound: 155 recorded before the run stopped. Unmeasured: Python populations, slow + torch (job `cancelled`)._
...
- **FAIL** `python::tests/bug_hunt_multinomial_summary_norm_strided_test` :: `test_multinomial_summary_norm_strides_correctly`
