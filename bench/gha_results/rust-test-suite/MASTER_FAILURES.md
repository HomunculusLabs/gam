...
- Python test failures: **NOT MEASURED — at least 150** (LOWER BOUND, not a count: Python populations, slow + torch (job `cancelled`) did not run to completion, so the tests they never reached are unmeasured, not passing)
...
- `?` — could not compile `gam-cli` (bin "gam") due to 1 previous error
...
_Lower bound: 150 recorded before the run stopped. Unmeasured: Python populations, slow + torch (job `cancelled`)._
