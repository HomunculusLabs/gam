...
- Python test failures: **NOT MEASURED — at least 149** (LOWER BOUND, not a count: Python populations, slow + torch (job `cancelled`) did not run to completion, so the tests they never reached are unmeasured, not passing)
...
> NOTE: the runtime surface was NOT measured — the shard matrix reported `cancelled`; only 9 of 10 planned shard logs were collected; a shard reported ARCHIVE_MISSING. Runtime counters above are not results. Fix the build first; the runtime surface will then be exercised.

## Compile failures

- `crates/gam-models/src/fit_orchestration/drivers/psi_gram_tensor_fast_path_tests.rs:185:55` — [E0277] `?` couldn't convert the error to `std::string::String`
- `crates/gam-models/src/fit_orchestration/drivers/psi_gram_tensor_fast_path_tests.rs:613:55` — [E0277] `?` couldn't convert the error to `std::string::String`
- `crates/gam-models/src/fit_orchestration/drivers/design_assembly_constraint_tests.rs:4407:55` — [E0277] `?` couldn't convert the error to `std::string::String`
- `crates/gam-models/src/fit_orchestration/drivers/design_assembly_constraint_tests.rs:4751:55` — [E0277] `?` couldn't convert the error to `std::string::String`
- `?` — could not compile `gam-models` (lib test) due to 4 previous errors
...
_Lower bound: 149 recorded before the run stopped. Unmeasured: Python populations, slow + torch (job `cancelled`)._
