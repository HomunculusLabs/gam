    ...
    'joint_newton_cycles: for cycle in 0..inner_loop_hard_ceiling {
        ...
        if returned_mode_curvature_pending {
            ...
            if has_resolvable_negative_curvature || !exact_first_order_certified {
                ...
                // certificate still has a resolvable Newton correction. Give
                // that correction a full trial under the existing acceptance
                // checks. Reusing the collapsed radius takes microscopic
                // steps, then repeats this same certificate indefinitely.
                if !has_resolvable_negative_curvature {
                    let correction_norm =
                        joint_trust_region_metric_step_norm(&delta, &joint_trust_metric_diag);
                    if correction_norm.is_finite() {
                        joint_trust_radius = joint_trust_radius.max(correction_norm);
                    }
                    let correction_blocks = joint_trust_region_block_metric_norms(
                        &delta,
                        &ranges,
                        &joint_trust_metric_diag,
                    );
                    for (radius, norm) in joint_block_trust_radii.iter_mut().zip(correction_blocks) {
                        if norm.is_finite() {
                            *radius = radius.max(norm);
                        }
                    }
                }
                converged = false;
