...
/// Schema inference and the schema-guided predict ingest
/// (`gam-pyffi::string_records_from_rows`) both strip this prefix before
/// recording or matching a level; its presence forces
...
fn infer_schema_column(
    ...
    // Canonical natural-sorted level order, so the inferred schema does not depend
    // on row order (#1319).
