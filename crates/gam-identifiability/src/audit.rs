...
//   3. `crate::families::compiler::compile` does the W-metric
...
// `DroppedColumn`, `IdentifiabilityAudit`) now live in `gam-problem` so the
// `CustomFamilyError` cone and other low-level consumers can name them without
// depending on this monolith. The compute code below builds them through their
// public fields exactly as before.
pub use gam_problem::{AliasedPair, BlockIdentity, DroppedColumn, IdentifiabilityAudit};
...
pub fn audit_identifiability_channel_aware(
    ...
    let ordering: Vec<crate::families::compiler::BlockOrder> =
        std::iter::repeat(crate::families::compiler::BlockOrder::Marginal)
            .take(operators.len())
            .collect();
...
// alongside the other pure-data audit result types. The compute below builds it
// through its public fields exactly as before.
pub use gam_problem::MapUniquenessError;
