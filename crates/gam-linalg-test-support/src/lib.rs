//! Numerical test fixtures shared across the workspace.
//!
//! Only tests use anything here, so none of it belongs in a production build.
//! This crate sits directly over `gam-linalg` and is pulled in as a
//! dev-dependency (or through `gam-test-support`), which keeps these helpers
//! out of every production artifact while a crate cross-checking a derivative
//! still avoids dragging the whole model layer into its test build.
//!
//! `gam-linalg`'s own unit tests cannot use this crate: a crate's unit tests
//! compile the crate itself separately, and a fixture built against the normal
//! `gam-linalg` returns types that are not the unit-test build's. The one lock
//! those tests share (`with_global_parallelism_serialized`) therefore stays in
//! `gam-linalg` under `cfg(test)`.
//!
//! [`fd_checker`] carries the central-difference derivative checks: `ndarray`
//! in, `ndarray` out, no model-layer type.
//!
...
//!
//! [`paired_holdout_partition`] is model-free deterministic numerical test
//! design. Keeping its row-ranking primitive here lets downstream quality tests
//! share one paired partition without making its invariant test compile the
//! reference bridge and the entire model stack.
//!
...
///
/// `#[macro_export]` puts this at the crate root, so the call path is
/// `gam_linalg_test_support::assert_central_difference_array!`. Its only
/// callers are `#[cfg(test)]` code, which is why it lives in this dev-only
/// crate: the #1440 production-FD ban reads a central-difference macro in a
/// production crate as production finite differencing.
#[macro_export]
