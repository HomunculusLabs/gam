...
impl Default for TailCellMomentCache {
    fn default() -> Self {
        // Tail-cell entries are small (a short moment vector), so sharding
        // the byte/entry budgets is harmless; size the shard count off the
        // worker pool exactly like the per-family cell-moment LRU.
        let shard_count = std::thread::available_parallelism()
            .map(|workers| workers.get().saturating_mul(8))
            .unwrap_or(32)
            .clamp(8, 256);
        Self {
            moments: ByteLruCache::with_max_entries_sharded(
...
static TAIL_CELL_MOMENT_CACHE: std::sync::OnceLock<TailCellMomentCache> =
    std::sync::OnceLock::new();

fn tail_cell_moment_cache() -> &'static TailCellMomentCache {
    TAIL_CELL_MOMENT_CACHE.get_or_init(TailCellMomentCache::default)
}
...
// Shared SIMD Gauss-Legendre core for non-affine cells. The const generic
// `COMPUTE_VALUE` selects whether the cell value integral
// `∫ φ(η(z)) · exp(-½z²) dz / √(2π)` is accumulated alongside the moments.
// Monomorphization collapses the const-generic branches at compile time, so
// `COMPUTE_VALUE = false` emits the moment-only path verbatim.
//
...
/// fine-rule error.
///
/// History (#979): a roundoff-floor relaxation of this test (accept when
/// successive rungs agree to `≈ n·ε·scale` rather than the bare `3e-15`) was
/// tried to let smooth cells certify below the terminal 384-node rung. It was
/// reverted: the value-bearing path carries `∫ φ(z)·Φ(η(z)) dz`, and `Φ`'s
/// `erfc` implementation has a *systematic per-z* error of order `1e-13` that
/// each rung's node set samples differently. Only the exact 384-node rule
...
pub fn evaluate_cell_moments(
    ...
    tail_cell_moment_cache().evaluate(cell, max_degree)
}
...
mod tests {
    ...
    //       third-derivative boundary integrand's flux formula is exact
