...
impl BernoulliMarginalSlopeFamily {
    ...
    pub(crate) fn exact_newton_joint_hessian_matvec_from_cache_into(
        ...
        // per-row matvec helper from `crate::gpu_kernels::row_hessian_ops`. On Linux this
    ...
    pub(super) fn exact_newton_joint_hessian_diagonal_from_cache(
        ...
        // per-row diagonal helper from `crate::gpu_kernels::row_hessian_ops`, then perform
    ...
    pub(super) fn run_psi_row_pass_for_axes(
        ...
        let folded = gam_linalg::pairwise_reduce::par_deterministic_try_block_fold(
            weighted_rows.len(),
    ...
    pub(crate) fn exact_newton_joint_psisecond_order_terms_from_cache_with_options(
        ...
        let (objective_psi_psi, score_psi_psi, block_acc) =
            gam_linalg::pairwise_reduce::par_deterministic_try_block_fold(
                weighted_rows.len(),
                |index_range| -> Result<_, String> {
                    let mut acc = make_acc();
