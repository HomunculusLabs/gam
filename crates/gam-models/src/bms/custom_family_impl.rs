...
use super::residual_repair_kernel::{
    ResidualDriveKernel, residual_hessian_dense, residual_hessian_directional_derivative,
    residual_hessian_directional_derivative_all_axes,
    residual_hessian_second_directional_derivative,
    residual_hessian_second_directional_derivative_all_axes,
};
...
impl BernoulliMarginalSlopeFamily {
    ...
    /// slope primaries carry design rows, while h/w primaries map directly
    ...
    fn fill_explicit_psi_primary_sandwich(
        ...
        slope_row: ndarray::ArrayView1<'_, f64>,
        marginal_weight_projection: ndarray::ArrayView1<'_, f64>,
        slope_weight_projection: ndarray::ArrayView1<'_, f64>,
        out: &mut [f64],
    ) -> Result<(), String> {
        let r = primary.total;
        let expected = r
        ...
        let mut qg = 0.0;
        for (local, &design_value) in slope_row.iter().enumerate() {
            qg += design_value * marginal_weight_projection[slices.slope.start + local];
        }
        let mut gg = 0.0;
        for (local, &design_value) in slope_row.iter().enumerate() {
            gg += design_value * slope_weight_projection[slices.slope.start + local];
        }
        out[primary.q * r + primary.q] = qq;
        out[primary.q * r + primary.slope] = qg;
        out[primary.slope * r + primary.q] = qg;
        out[primary.slope * r + primary.slope] = gg;
        ...
        for &(primary_idx, block_idx) in tail_pairs {
            ...
            let g_tail = slope_weight_projection[block_idx];
            out[primary.slope * r + primary_idx] = g_tail;
            out[primary_idx * r + primary.slope] = g_tail;
        }
        Ok(())
    }
    ...
    fn explicit_psi_jeffreys_mixed_row_pullback(
        ...
        let (axis_range, expected_primary) = match axis.block_idx {
            ...
            1 => (slices.slope.clone(), primary.slope),
            block => {
                return Err(format!(
                    "BMS explicit-psi Jeffreys pullback only supports marginal/slope axes, got block {block}"
                ));
            }
        };
        ...
        let panel_width = 5usize
            ...
            .saturating_add(slices.slope.len())
        ...
        let pulled = gam_linalg::pairwise_reduce::par_deterministic_try_block_fold_by_work(
            ...
            |chunk_range| -> Result<Array1<f64>, String> {
                ...
                for chunk_index in chunk_range {
                    ...
                    let slope_owned;
                    let slope = match self.slope_design.as_dense_ref() {
                        Some(dense) => dense.slice(s![rows.clone(), ..]),
                        None => {
                            slope_owned = self
                                .slope_design
                                ...
                                .map_err(|error| {
                                    format!(
                                        "BMS explicit-psi Jeffreys slope panel failed: {error}"
                                    )
                                })?;
                            slope_owned.view()
                        }
                    ...
                    let (a_m, a_g, b_m, b_g, b_r) = gam_problem::with_nested_parallel(|| {
                        (
                            ...
                            gam_linalg::faer_ndarray::fast_ab(&slope, &a_g_block),
                            gam_linalg::faer_ndarray::fast_ab(&marginal, &b_m_block),
                            gam_linalg::faer_ndarray::fast_ab(&slope, &b_g_block),
                    ...
                    for local in 0..(end - start) {
                        ...
                        for (offset, &design_value) in slope_row.iter().enumerate() {
                            cross_primary[primary.slope] +=
                                design_value * b_r_row[slices.slope.start + offset];
                        }
                        for &(primary_idx, block_idx) in &tail_pairs {
...
impl CustomFamily for BernoulliMarginalSlopeFamily {
    ...
    /// marginal and slope surfaces share one matern PC basis, so even with
    ...
    /// exhausts, WITHOUT moving the converged β (the link-deviation / slope
    ...
    fn exact_newton_joint_hessian(
        ...
        if self.residual_active() {
            let kern = ResidualDriveKernel::new(self.clone(), block_states.to_vec())?;
            let cache = build_row_kernel_cache(&kern, &crate::row_kernel::RowSet::All)?;
            return residual_hessian_dense(&kern, &cache).map(Some);
        }
        if !self.effective_flex_active(block_states)? {
        ...
        // every (a, b) entry into the marginal/slope/h/w block targets.
    ...
    fn exact_newton_joint_gradient_evaluation(
        ...
        self.validate_exact_monotonicity(block_states)?;
        if self.residual_active() {
            // The tangent Jacobian's `v` row is `2Σ_rrβ`, so the generic
            // gradient pullback is exact; only the Hessian channels carry the
            // curvature of `v`.
            let kern = ResidualDriveKernel::new(self.clone(), block_states.to_vec())?;
            let cache = build_row_kernel_cache(&kern, &crate::row_kernel::RowSet::All)?;
    ...
    fn exact_newton_joint_hessian_workspace(
        ...
        if self.residual_active() {
            // The residual kernel's Hessian carries the curvature of its
            // quadratic primary, which the generic matrix-free workspace does
            // not see; the exact dense channels serve the solver instead.
            return Ok(None);
        }
        if !self.effective_flex_active(block_states)? {
    ...
    fn exact_newton_joint_hessian_workspace_with_options(
        ...
        if self.residual_active() {
            // See `exact_newton_joint_hessian_workspace`: the residual block is
            // served by the exact dense channels, full data only.
            return Ok(None);
        }
        if !self.effective_flex_active(block_states)? {
    ...
    fn inner_coefficient_hessian_hvp_available(&self, specs: &[ParameterBlockSpec]) -> bool {
        // The workspace impl above returns `Some(workspace)` on the rigid path
        // (a `RowKernelHessianWorkspace`) and the flex path (a
        // `BernoulliMarginalSlopeExactNewtonJointHessianWorkspace`), and both
        // route the joint Hessian through Hv operators rather than dense
        // assembly. With the residual block it returns `None` and the dense
        // channels serve the solve, so no Hv workspace is advertised then.
        // This advertises β-space representation support only; it does not
        // imply a profiled outer θ-HVP.
        !self.residual_active() && parameter_block_specs_match_rows(specs, self.y.len())
    }
    ...
    // The residual block builds no workspace: its gradient and log-likelihood
    // come from `exact_newton_joint_gradient_evaluation` and `evaluate`.
    ...
    fn exact_newton_joint_hessian_directional_derivative(
        ...
        if self.residual_active() {
            ...
            let kern = ResidualDriveKernel::new(self.clone(), block_states.to_vec())?;
            return residual_hessian_directional_derivative(&kern, sl).map(Some);
        }
        if !self.effective_flex_active(block_states)? {
            let kern = BernoulliRigidRowKernel::new(self.clone(), block_states.to_vec());
            let sl = d_beta_flat.as_slice().ok_or("non-contiguous d_beta")?;
    ...
    fn exact_newton_joint_hessiansecond_directional_derivative(
        ...
        if self.residual_active() {
            ...
            let kern = ResidualDriveKernel::new(self.clone(), block_states.to_vec())?;
            return residual_hessian_second_directional_derivative(&kern, su, sv).map(Some);
        }
        if !self.effective_flex_active(block_states)? {
            let kern = BernoulliRigidRowKernel::new(self.clone(), block_states.to_vec());
            let su = d_beta_u_flat.as_slice().ok_or("non-contiguous d_beta_u")?;
    ...
    fn joint_jeffreys_information_directional_derivative_all_axes_with_specs(
        ...
        if self.residual_active() {
            let kern = ResidualDriveKernel::new(self.clone(), block_states.to_vec())?;
            return residual_hessian_directional_derivative_all_axes(&kern).map(Some);
        }
    ...
    fn joint_jeffreys_information_second_directional_all_axes_with_specs(
        ...
        if self.residual_active() {
            ...
            let kern = ResidualDriveKernel::new(self.clone(), block_states.to_vec())?;
            return residual_hessian_second_directional_derivative_all_axes(&kern, su).map(Some);
        }
    ...
    fn joint_jeffreys_information_contracted_trace_hessian_with_specs(
        ...
        if self.residual_active() {
            ...
            for (index, &scale) in scales.iter().enumerate() {
                ...
                let second =
                    residual_hessian_second_directional_derivative(&kern, direction, direction)?;
...
impl BernoulliMarginalSlopeFamily {
    ...
    pub(super) fn row_factor_primary_projection(
        ...
        for col in 0..rank {
            ...
            out[primary.slope * rank + col] = self
                .slope_design
                .dot_row_view(row, factor.slice(s![slices.slope.clone(), col]));
        }
            ...
            |index_range| -> Result<Vec<f64>, String> {
                ...
                for wr in &weighted_rows[index_range] {
                    ...
                    for dir_idx in 0..n_dirs {
                        ...
                        trace += trace_gradient[primary.slope]
                            * self.slope_design.dot_row_view(
                                row,
                                directions.slice(s![slices.slope.clone(), dir_idx]),
                            );
    ...
    pub(super) fn batched_rho_correction_logdet_traces_full_rows(
        ...
        let dir_g = directions.slice(s![slices.slope.clone(), ..]);
        let tail_pairs = Self::primary_tail_block_pairs(slices, primary);
        ...
        for chunk_idx in 0..n_chunks {
            ...
            let chunk_acc = gam_linalg::pairwise_reduce::par_deterministic_try_block_fold(
                ...
                for local in local_range {
                    ...
                    gram[primary.q * primary.total + primary.slope] = qg;
                    gram[primary.slope * primary.total + primary.q] = qg;
                    gram[primary.slope * primary.total + primary.slope] = gg;
                    for &(primary_idx, block_idx) in &tail_pairs {
                        let mut q_tail = 0.0;
                        let mut g_tail = 0.0;
                        for col in 0..rank {
                        ...
                        gram[primary.slope * primary.total + primary_idx] = g_tail;
                        gram[primary_idx * primary.total + primary.slope] = g_tail;
                    }
                    let row_ctx = Self::row_ctx(cache, row);
                    if n_dirs == 1 {
                        row_dir.fill(0.0);
                        row_dir[primary.q] = dir_proj_m[[local, 0]];
                        row_dir[primary.slope] = dir_proj_g[[local, 0]];
                        if let (Some(block_range), Some(primary_range)) =
                            (slices.h.as_ref(), primary.h.as_ref())
                        {
                            for (offset, block_idx) in block_range.clone().enumerate() {
                    ...
                    for dir_idx in 0..n_dirs {
                        let mut trace = trace_gradient[primary.q] * dir_proj_m[[local, dir_idx]]
                            + trace_gradient[primary.slope] * dir_proj_g[[local, dir_idx]];
                        if let (Some(block_range), Some(primary_range)) =
                            (slices.h.as_ref(), primary.h.as_ref())
                        {
                            for (offset, block_idx) in block_range.clone().enumerate() {
...
impl gam_problem::ExactNewtonJointPsiAxisContractions
