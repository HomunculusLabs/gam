...
pub(crate) fn find_mode(
    ...
    for _ in 0..200 {
        ...
        if decrement <= floor {
            // Below the roundoff floor of `F` the full step is exact to
            // roundoff; taking it once more puts the mode at machine
            // precision rather than at the floor's square root.
        ...
        loop {
            ...
            {
                let gain = candidate.objective - assembly.objective;
                z = trial;
                assembly = candidate;
                stalls = if gain <= floor { stalls + 1 } else { 0 };
                break;
            }
            t *= 0.5;
            if t < 1e-12 {
