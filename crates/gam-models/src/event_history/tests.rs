...
fn family_joint_hessian_matches_finite_differences_of_its_gradient() {
    ...
    // Two atoms at fixed rates: the latent block is the loadings alone.
...
fn four_marks_recover_a_shared_latent_covariance() {
    ...
    let family0 =
        EventHistoryFamily::new(Arc::clone(&nodes), vec![Arc::clone(&dense); marks], Vec::new(), cohort.time_scale())
            .expect("family");
    ...
    // The simulated rates are 0.3 and 1.5 per unit against a mean follow-up
    // near 9, so the faster atom sits just under what the coarse mesh can
    // resolve; one refinement puts both inside it.
    let mut spec = EventHistorySpec::new(vec![linear_spec()]);
    spec.mesh_refinement = 1;
    let fit = fit_event_history(&mut cohort, &spec).expect("fit");
    let covariance = fit.disease_covariance();
    ...
    // Every step reached a certified optimum, so every judgement the rank
    // path reports is one the evidence actually made. An atom held at the
    // mesh's resolution limit is reported, not refused: it says the
    // residuals carry structure the mesh only just resolves.
    for step in &fit.rank_path {
        assert!(step.converged, "a rank step reported no certified optimum: {step:?}");
    }
