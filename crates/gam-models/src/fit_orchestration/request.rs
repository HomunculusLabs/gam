...
pub enum FitResult {
    /// Complete CTN/outcome predictor with frozen native transformation state.
...
/// Native cross-fitted conditional score recipe. Each fold learns its own
/// geometry; a full-training CTN is saved for deployment. No influence absorber.
#[derive(Clone, Debug)]
pub struct CtnStage1Recipe {
    /// Explicit nuisance-fold column, optionally checked against groups.
    pub fold_column: Option<String>,
    /// Independent groups used for reproducible whole-group folds.
    pub group_column: Option<String>,
    /// Number of generated group folds.
    pub folds: usize,
    /// Reproducible group-fold seed.
    pub seed: u64,
    /// Stage-1 response column name (the `y` the CTN transforms).
    pub response_column: String,
    /// Covariate formula RHS. Each nuisance fold learns its own geometry.
    pub covariate_formula_rhs: String,
    /// Existing CTN response basis and function-penalty configuration.
    pub config: TransformationNormalConfig,
...
impl CtnStage1Recipe {
    ...
    /// call [`fit_from_formula`] for the complete native predictor. Cross-fitting
    /// does not install an influence absorber or establish orthogonality.
    ///
...
pub struct FitConfig {
    ...
    /// Mutually exclusive with cross-fitting and an external latent-score column.
    pub frozen_ctn: Option<crate::inference::ctn::FrozenCtn>,
