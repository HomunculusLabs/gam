...
mod observed_single_source_oracle_tests {
    ...
    mod jet_third_fourth_oracle {
        ...
        fn release_measure_gaussian_joint_specialized_vs_generic_tower_932() {
            ...
            // the batch; the batch carries distinct rows and the nudge perturbs
            // each row's direction (or the fold, for the direction-free
            // order-2 sweep), so no sweep is loop-invariant across calls.
            if cfg!(debug_assertions) {
                return;
            }
            let mut gate = SpeedGate::open("GAUSSIAN-JOINT-RELEASE-932");
            let reps = 15usize;
            let sweeps = 20usize;
            ...
            let third = paired_interleaved(
                ...
                |nudge| {
                    ...
                    for row in 0..ROWS {
                        let mut direction = dir_u[row];
                        direction[0] += nudge;
                        let third = program.row_third_contracted(row, &direction);
                ...
                |nudge| {
                    ...
                    for row in 0..ROWS {
                        let mut direction = dir_u[row];
                        direction[0] += nudge;
                        let third = program_third_contracted(&program, row, &direction)
                            .expect("tower third");
                        checksum += third[0][0] + third[0][1] + third[1][1];
                    }
                    checksum
            ...
            let fourth = paired_interleaved(
                ...
                |nudge| {
                    ...
                    for row in 0..ROWS {
                        let mut direction = dir_u[row];
                        direction[0] += nudge;
                        let fourth = program.row_fourth_contracted(row, &direction, &dir_v[row]);
                ...
                |nudge| {
                    ...
                    for row in 0..ROWS {
                        ...
                        direction[0] += nudge;
                        let fourth =
                            program_fourth_contracted(&program, row, &direction, &dir_v[row])
                                .expect("tower fourth");
                        checksum += fourth[0][0] + fourth[0][1] + fourth[1][1];
                    }
                    checksum
