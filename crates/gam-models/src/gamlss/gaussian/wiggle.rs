...
/// Bitmask of the three coefficient directions `u = 1`, `v = 2`, `w = 4` of a
/// third directional derivative.
const ALL_DIRECTIONS: usize = 0b111;

/// `D_S(x·y)` along the direction set `S` (a bitmask over `u = 1`, `v = 2`,
/// `w = 4`), from each factor's derivatives along every subset of `S`: the
/// Leibniz sum `Σ_{T ⊆ S} x_T·y_{S∖T}`.
fn directional_leibniz(x: &[f64; 8], y: &[f64; 8], set: usize) -> f64 {
    let mut total = 0.0;
    let mut subset = set;
    loop {
        total += x[subset] * y[set ^ subset];
        if subset == 0 {
            return total;
        }
        subset = (subset - 1) & set;
...
/// The derivatives of `x·y` along every subset of the three directions.
fn leibniz_jet(x: &[f64; 8], y: &[f64; 8]) -> [f64; 8] {
    std::array::from_fn(|set| directional_leibniz(x, y, set))
}

/// The derivative along the direction set `S ≠ ∅` of the warp quantity
/// `∂ᵐq/∂q0ᵐ` at one row, with `m = level` (`m = 0` is `q` itself). `xi[S]` is
/// `Π_{d∈S} ξ_d` with `ξ_d = X_mu·δβ_mu[d]`, `warp[k]` is `∂ᵏ⁺¹q/∂q0ᵏ⁺¹` and
/// `directions[d][k]` is `B⁽ᵏ⁾·δβ_w[d]`. Because `q0` is linear in `β` and `q`
/// is linear in `β_w`, with `k = m + |S| − 1`,
/// `D_S ∂ᵐq/∂q0ᵐ = warp[k]·xi[S] + Σ_{d∈S} directions[d][k]·xi[S∖{d}]`.
fn wiggle_warp_leg(
    warp: &[f64; 5],
    xi: &[f64; 8],
    directions: &[[f64; 5]; 3],
    ...
    let order = level + set.count_ones() as usize - 1;
    let mut total = warp[order] * xi[set];
    for (direction, contraction) in directions.iter().enumerate() {
        let bit = 1 << direction;
        if set & bit != 0 {
            total += contraction[order] * xi[set ^ bit];
        }
    }
    total
}
...
impl GaussianLocationScaleWiggleFamily {
    ...
    /// `{D³H[u, v, e_a]}` over every coefficient axis `a`: the β-derivative of
    /// [`Self::exact_newton_joint_hessiansecond_directional_derivative_from_designs`]
    /// along each axis, which an armed Jeffreys objective's exact outer Hessian
    /// reads (#2677). The row channels come from the generated fifth
    /// contraction, and the warp legs reach `B⁽⁴⁾` and `d⁵q/dq0⁵`.
    ///
    /// Every block is a sum of `Xᵀ diag(c) Y` over the channels `X_mu`, `X_ls`
    /// and `B⁽ᵏ⁾`. The wiggle basis moves with `q0`, so its derivative along a
    /// direction set `S` is `diag(ξ_S)·B⁽|S|⁾`, and the row factor `ξ_S` folds
    /// into `c`: one product per pair of basis orders rather than one per term
    /// of the Leibniz expansion.
    pub(crate) fn exact_newton_joint_hessian_third_directional_all_axes_from_designs(
        &self,
        block_states: &[ParameterBlockState],
        xmu: &Array2<f64>,
        x_ls: &Array2<f64>,
        ...
        let n = self.y.len();
        let pw = betaw.len();
        let layout = GamlssBetaLayout::withwiggle(pmu, p_ls, pw);
        let (umu, u_ls, uw) = layout.split_three(
            d_beta_u_flat,
            "GaussianLocationScaleWiggleFamily exact joint third directional Hessian (u)",
        )?;
        let (vmu, v_ls, vw) = layout.split_three(
            d_beta_v_flat,
            "GaussianLocationScaleWiggleFamily exact joint third directional Hessian (v)",
        )?;
        if q0.len() != n || eta_ls.len() != n || etaw.len() != n || self.weights.len() != n {
            return Err(GamlssError::DimensionMismatch {
                reason: "GaussianLocationScaleWiggleFamily input size mismatch".to_string(),
        ...
        let q = q0 + etaw;
        let geom = self.wiggle_geometry(q0.view(), betaw.view())?;
        let basis_d4 = self.wiggle_d4basis_constrained(q0.view())?;
        let d5q_dq05 = self.wiggle_d5q_dq05(q0.view(), betaw.view())?;
        let rows = self.get_or_compute_row_scalars(&q, eta_ls)?;
        ...
        let xi_uv = [fast_av(xmu, &umu), fast_av(xmu, &vmu)];
        let zeta_uv = [fast_av(x_ls, &u_ls), fast_av(x_ls, &v_ls)];
        let wiggle_uv = [
            bases.map(|basis| fast_av(basis, &uw)),
            bases.map(|basis| fast_av(basis, &vw)),
        ];
        let p = pmu + p_ls + pw;
        let mut axes = Vec::with_capacity(p);
        for axis in 0..p {
            let mut coeff_mm = Array1::<f64>::zeros(n);
            let mut coeff_ml = Array1::<f64>::zeros(n);
            let mut coeff_ll = Array1::<f64>::zeros(n);
            // `[k, row]`: the coefficient of `X_muᵀ diag(·) B⁽ᵏ⁾` and of `X_lsᵀ diag(·) B⁽ᵏ⁾`.
            let mut mean_wiggle = Array2::<f64>::zeros((5, n));
            let mut scale_wiggle = Array2::<f64>::zeros((4, n));
            // `[i, j, row]`, `i ≤ j`: the coefficient of `B⁽ⁱ⁾ᵀ diag(·) B⁽ʲ⁾`.
            let mut wiggle_wiggle = ndarray::Array3::<f64>::zeros((2, 4, n));
            for row in 0..n {
                // The axis is the third direction `w`: one design column of its block.
                let (xi_w, zeta_w, wiggle_w) = if axis < pmu {
                    (xmu[[row, axis]], 0.0, [0.0; 5])
                } else if axis < pmu + p_ls {
                    (0.0, x_ls[[row, axis - pmu]], [0.0; 5])
                } else {
                    let column = axis - pmu - p_ls;
                    (0.0, 0.0, bases.map(|basis| basis[[row, column]]))
                };
                let xi_u = xi_uv[0][row];
                let xi_v = xi_uv[1][row];
                let xi = [
                    1.0,
                    xi_u,
                    xi_v,
                ...
                let zeta = [zeta_uv[0][row], zeta_uv[1][row], zeta_w];
                let warp = [
                    geom.dq_dq0[row],
                    geom.d2q_dq02[row],
                    geom.d3q_dq03[row],
                ...
                let directions = [
                    std::array::from_fn(|order| wiggle_uv[0][order][row]),
                    std::array::from_fn(|order| wiggle_uv[1][order][row]),
                    wiggle_w,
                ];
                // `ℓ` is linear in `β`, so only a single direction moves it.
                let legs: [[f64; 2]; 8] = std::array::from_fn(|set| {
                    if set == 0 {
                        [0.0, 0.0]
                    } else if set.count_ones() == 1 {
                        [
                            wiggle_warp_leg(&warp, &xi, &directions, 0, set),
                            zeta[set.trailing_zeros() as usize],
                        ]
                    } else {
                        [wiggle_warp_leg(&warp, &xi, &directions, 0, set), 0.0]
                    }
                });
                let (gradients, hessians) = program.row_third_tower(row, &legs);
                let slope: [f64; 8] = std::array::from_fn(|set| gradients[set][0]);
                let curvature: [f64; 8] = std::array::from_fn(|set| hessians[set][0][0]);
                let cross: [f64; 8] = std::array::from_fn(|set| hessians[set][0][1]);
                let dq: [f64; 8] = std::array::from_fn(|set| {
                    if set == 0 {
                        warp[0]
                    } else {
                        wiggle_warp_leg(&warp, &xi, &directions, 1, set)
                    }
                });
                let d2q: [f64; 8] = std::array::from_fn(|set| {
                    if set == 0 {
                        warp[1]
                    } else {
                        wiggle_warp_leg(&warp, &xi, &directions, 2, set)
                    }
                });
                // coeff_mm = f_qq·d² + f_q·d2q, coeff_ml = f_qℓ·d, coeff_ll = f_ℓℓ.
                coeff_mm[row] =
                    directional_leibniz(&curvature, &leibniz_jet(&dq, &dq), ALL_DIRECTIONS)
                        + directional_leibniz(&slope, &d2q, ALL_DIRECTIONS);
                coeff_ml[row] = directional_leibniz(&cross, &dq, ALL_DIRECTIONS);
                coeff_ll[row] = hessians[ALL_DIRECTIONS][1][1];
                // h_mw = X_muᵀ diag(f_qq·d) B + X_muᵀ diag(f_q) B', h_lw = X_lsᵀ diag(f_qℓ) B:
                // the factor along `S` meets the basis along the complement.
                let mean = leibniz_jet(&curvature, &dq);
                for set in 0..8usize {
                    let complement = ALL_DIRECTIONS ^ set;
                    let order = complement.count_ones() as usize;
                    mean_wiggle[[order, row]] += mean[set] * xi[complement];
                    mean_wiggle[[order + 1, row]] += slope[set] * xi[complement];
                    scale_wiggle[[order, row]] += cross[set] * xi[complement];
                }
                // h_ww = Bᵀ diag(f_qq) B: `S_L`, `S_W`, `S_R` partition the directions.
                for left in 0..8usize {
                    let remaining = ALL_DIRECTIONS ^ left;
                    let mut right = remaining;
                    loop {
                        let left_order = left.count_ones() as usize;
                        let right_order = right.count_ones() as usize;
                        if left_order <= right_order {
                            wiggle_wiggle[[left_order, right_order, row]] +=
                                xi[left] * curvature[remaining ^ right] * xi[right];
                        }
                        if right == 0 {
                            break;
                        }
            ...
            let mut h_mw = Array2::<f64>::zeros((pmu, pw));
            for (order, basis) in bases.iter().enumerate() {
                h_mw += &xt_diag_y_dense(xmu, &mean_wiggle.row(order).to_owned(), basis)?;
            }
            let mut h_lw = Array2::<f64>::zeros((p_ls, pw));
            for (order, basis) in bases.iter().take(4).enumerate() {
                h_lw += &xt_diag_y_dense(x_ls, &scale_wiggle.row(order).to_owned(), basis)?;
            }
            let mut h_ww = Array2::<f64>::zeros((pw, pw));
            for left_order in 0..2usize {
                for right_order in left_order..(4 - left_order) {
                    let block = xt_diag_y_dense(
                        bases[left_order],
                        &wiggle_wiggle
                            .slice(ndarray::s![left_order, right_order, ..])
                            .to_owned(),
                        bases[right_order],
                    )?;
                    h_ww += &block;
            ...
            axes.push(gaussian_pack_wiggle_joint_symmetrichessian(
                &h_mm, &h_ml, &h_mw, &h_ll, &h_lw, &h_ww,
            ));
        }
        Ok(axes)
    ...
    pub(crate) fn exact_newton_joint_psisecond_order_terms_from_parts(
        ...
        let GlsWiggleSecondDirCoeffs {
            ...
            coeff_mm_base: coeff_mm,
            coeff_mm_u: coeff_mm_a,
            coeff_mm_v: coeff_mm_b,
            coeff_mm_uv: coeff_mm_ab,
            coeff_ml_base: coeff_ml,
            coeff_ml_u: coeff_ml_a,
            coeff_ml_v: coeff_ml_b,
            coeff_ml_uv: coeff_ml_ab,
            coeff_ll_base: coeff_ll,
            coeff_ll_u: coeff_ll_a,
            coeff_ll_v: coeff_ll_b,
            coeff_ll_uv: coeff_ll_ab,
            mean_wiggle_base: a,
            a_u: a_a,
            a_v: a_b,
            a_uv: a_ab,
            ...
            gradient_ls_base: s_ls,
            gradient_ls_u: s_ls_a,
            gradient_ls_v: s_ls_b,
            gradient_ls_uv: s_ls_ab,
            hessian_ml_base: l,
            l_u: l_a,
            l_v: l_b,
            l_uv: l_ab,
            hessian_mm_base,
            hessian_mm_u: hessian_mm_a,
            hessian_mm_v: hessian_mm_b,
            hessian_mm_uv: hessian_mm_ab,
        } = gls_wiggle_second_directional_coeffs(
            &rows,
            &geom,
            &GlsWiggleDirPieces {
        ...
        let s_mu = &c * &geom.dq_dq0;
        let s_mu_a = &c_a * &geom.dq_dq0 + &c * &s1_a;
        let s_mu_b = &c_b * &geom.dq_dq0 + &c * &s1_b;
        let s_mu_ab = &c_ab * &geom.dq_dq0 + &c_a * &s1_b + &c_b * &s1_a + &c * &s1_ab;

        let objective_psi_psi = objective_uv.sum();
        ...
        let score_psi_psi = gaussian_pack_wiggle_joint_score(
            ...
            &(fast_atv(&basis_ab, &c)
                + fast_atv(&basis_a, &c_b)
                + fast_atv(&basis_b, &c_a)
                + fast_atv(&geom.basis, &c_ab)),
        );
        ...
        let hww_ij = xt_diag_y_dense(&basis_a, &hessian_mm_base, &basis_b)?;
        let hww_iwj = xt_diag_y_dense(&basis_a, &hessian_mm_b, &geom.basis)?;
        let hww_jwi = xt_diag_y_dense(&basis_b, &hessian_mm_a, &geom.basis)?;
        let h_ww = &hww_ab
            + &hww_ab.t()
            + &hww_ij
            + hww_ij.t()
            ...
            + &hww_jwi
            + hww_jwi.t()
            + &xt_diag_x_dense(&geom.basis, &hessian_mm_ab)?;
...
impl crate::custom_family::JeffreysThirdInformationDerivative
    for GaussianLocationScaleWiggleFamily
{
    fn third_directional_all_axes(
        ...
        self.exact_newton_joint_hessian_third_directional_all_axes_from_designs(
            block_states,
            &xmu,
            &x_ls,
            d_beta_u_flat,
...
impl CustomFamily for GaussianLocationScaleWiggleFamily {
    ...
    /// `{D³H[u, v, e_a]}` from the generated fifth row contraction and the warp
    /// through `B⁽⁴⁾` and `d⁵q/dq0⁵` (#2677).
