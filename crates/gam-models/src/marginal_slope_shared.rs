...
/// `bail_invalid_estim!`).
pub(crate) fn make_beta_seed_validator(
...
#[inline]
pub(crate) const fn eval_coeff4_at(coefficients: &[f64; 4], z: f64) -> f64 {
...
#[inline]
pub(crate) const fn scale_coeff4(source: [f64; 4], scale: f64) -> [f64; 4] {
    [
        source[0] * scale,
        source[1] * scale,
        source[2] * scale,
...
pub(crate) const AUTO_OUTER_WORK_BUDGET: u64 = 500_000_000;
...
pub(crate) const AUTO_OUTER_MIN_K_FLOOR: usize = 1_000;
...
pub(crate) enum AutoOuterCapReason {
    Noise,
    Work,
    Floor,
    NFull,
...
pub(crate) struct AutoOuterKChoice {
    pub k: usize,
    pub k_noise: usize,
    pub k_work: usize,
...
pub(crate) fn auto_outer_target_k(n: usize, outer_work_per_k_unit: u64) -> Option<AutoOuterKChoice> {
...
pub(crate) fn auto_outer_score_subsample(
    z: &[f64],
...
pub(crate) fn build_outer_score_subsample(
    z: &[f64],
...
pub(crate) fn outer_row_indices(
...
pub(crate) fn outer_weighted_rows(
    opts: &crate::custom_family::BlockwiseFitOptions,
    n: usize,
...
pub(crate) fn outer_row_weights_by_index(
    opts: &crate::custom_family::BlockwiseFitOptions,
    n: usize,
) -> Vec<f64> {
    match opts.outer_score_subsample.as_ref() {
...
pub(crate) struct MarginalSlopeExactNewtonPsiWorkspace<F: MarginalSlopePsiFamily> {
    family: F,
}
...
// Regression guards for the large-scale 8h-hang investigation, moved here from
// the root `regressions` binary (`misc::subsample_k_cap_and_audit`) so the
// auto outer-subsample items they exercise stay crate-private.
//
// `maybe_install_auto_outer_subsample` used to build its subsample options from
// the defaults and discard the `outer_work_per_k_unit` argument. At large-scale
// n the noise-only rule then picked K ≈ 0.10·n = 19_661, ~9× larger than the
// survival family's intended K ≈ 2_000 at the time. The first outer analytic
// Hessian evaluation then exhausted RAM and disk; the kernel killed the process
// with SIGKILL (exit 137) after 8h of silent grinding. These tests guard that
// fix and document the K-cap multiplier quantitatively.
#[cfg(test)]
mod subsample_k_cap_tests {
    use super::*;
    use std::sync::Mutex;
    ...
    fn h1c_maybe_install_honors_outer_work_per_k_unit_argument() {
        // Regression guard for the K-cap bypass in maybe_install_auto_outer_subsample.
        ...
        let opts_struct = crate::custom_family::BlockwiseFitOptions {
...
/// Horvitz-Thompson semantics of the stratified outer-score subsample and its
/// row helpers, beside the crate-private helpers they pin (moved from
/// tests/inference/misc/subsample_outer_integration.rs and
/// tests/regressions/families/family_suite.rs, #2899).
#[cfg(test)]
mod outer_row_tests {
    use super::*;
