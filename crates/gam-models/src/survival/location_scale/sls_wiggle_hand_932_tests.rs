...
/// The warp towers along `u` and `v` and their fourth contraction.
#[inline(never)]
fn warp_pair(
    kw: usize,
    linear: Option<usize>,
...
/// The `qdot0` towers along `u` and `v` and its fourth contraction. Its nonzero fourth
/// partials are `(x6, x6, x6, x6) = qdot0`, `(x3, x6, x6, x6) = −s6·x8`,
/// `(x5, x6, x6, x6) = s6`, `(x6, x6, x6, x8) = −s6·x3` and `(x3, x6, x6, x8) = s6`.
#[inline(never)]
fn rate_index_pair(
    kw: usize,
    p: &[f64; SLS_ROW_K],
    ...
    rate_index_tower(kw, p, s6, qdot0, u, &mut pair.along_u);
    rate_index_tower(kw, p, s6, qdot0, v, &mut pair.along_v);
    ...
    add_symmetric(
        ...
        6,
        6,
        qdot0 * u[6] * v[6] - s6 * x8 * (u[3] * v[6] + u[6] * v[3])
            + s6 * (u[5] * v[6] + u[6] * v[5])
            - s6 * x3 * (u[6] * v[8] + u[8] * v[6])
            + s6 * (u[3] * v[8] + u[8] * v[3]),
    );
    add_symmetric(fourth, kw, 3, 6, -s6 * x8 * u[6] * v[6] + s6 * (u[6] * v[8] + u[8] * v[6]));
    add_symmetric(fourth, kw, 5, 6, s6 * u[6] * v[6]);
    add_symmetric(fourth, kw, 6, 8, -s6 * x3 * u[6] * v[6] + s6 * (u[3] * v[6] + u[6] * v[3]));
    add_symmetric(fourth, kw, 3, 8, s6 * u[6] * v[6]);
}

/// The towers of `g = x2 + m·r` along `u` and `v`, and its fourth contraction by the
/// sixteen-term product rule. Each of the four indices `a, b, u, v` falls on `m` or `r`:
...
fn hand_sls_wiggle_row_fourth(
    ...
    fourth.resize(kw * kw, 0.0);
    if entry_active {
        warp_pair(kw, Some(0), 4, 7, s7, q0, entry_derivs, [b0[0], b0[1], b0[2], b0[3]], u, v, entry);
        add_composition_fourth(fourth, kw, entry_outer, entry, u, v, work);
    }
    if censored_active || pdf_active {
        warp_pair(kw, Some(1), 3, 6, s6, q1, exit_derivs, [b1[0], b1[1], b1[2], b1[3]], u, v, exit);
    ...
    if rate_active {
        ...
        rate_index_pair(kw, p, s6, qdot0, u, v, rate_index);
        rate_pair(kw, exit_derivs[0], qdot0, multiplier, rate_index, u, v, work, rate);
