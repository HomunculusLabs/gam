...
/// Witnesses the anchored frame's tests read: the `∂α/∂b` jet a
/// follow-up-varying anchored frame would carry, a Gauss–Hermite law, and a
/// skewed law nothing Gaussian describes.
#[cfg(test)]
pub(crate) mod test_support {
    ...
    pub(crate) fn anchor_b_derivative_jet<T: JetField>(alpha: &T, q: &T, b: &T, grid: AnchorGrid<'_>) -> T {
        let mut density = q.constant_like(0.0);
...
#[cfg(test)]
mod anchor_tests {
    use super::test_support::{anchor_b_derivative_jet, gauss_hermite_probabilists, skewed_grid};
    use super::*;
