...
impl SurvivalMarginalSlopeRowKernel<DYNAMIC_SLOPE_PRIMARIES, DynamicSlopeGeometry> {
    ...
    /// [`Self::design_psi_third_information_all_axes_from`] on the follow-up-varying slope frame.
...
fn contract_fifth_all_primary_axes<const P: usize>(
    ...
    if dir_u.len() != P || dir_v.len() != P {
        return Err(format!(
            "rigid fifth contraction needs primary directions of length {P}, got {} and {}",
            dir_u.len(),
            dir_v.len()
        ));
    }
    Ok((0..P)
        .map(|k| {
            Array2::from_shape_fn((P, P), |(a, b)| {
                ...
                for c in 0..P {
                    for d in 0..P {
                        value += fifth[a][b][c][d][k] * dir_u[c] * dir_v[d];
                    }
                }
                value
            })
