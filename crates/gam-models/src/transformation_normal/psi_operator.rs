...
impl TensorKroneckerPsiOperator {
    ...
    pub(crate) fn cov_deriv(
        &self,
        axis: usize,
    ) -> Result<&CustomFamilyBlockPsiDerivative, gam_terms::basis::BasisError> {
        self.covariate_derivs.get(axis).ok_or_else(|| {
            gam_terms::basis::BasisError::InvalidInput(format!(
    ...
    pub(crate) fn cov_forward_first(
        ...
        if deriv.x_psi.nrows() == self.n_data() && deriv.x_psi.ncols() == self.p_cov() {
            return Ok(gam_linalg::faer_ndarray::fast_av(&deriv.x_psi, u));
        }
        let Some(op) = deriv.implicit_operator.as_ref() else {
            return Err(gam_terms::basis::BasisError::InvalidInput(format!(
    ...
    pub(crate) fn cov_transpose_first(
        ...
        let deriv = self.cov_deriv(axis)?;
        if deriv.x_psi.nrows() == self.n_data() && deriv.x_psi.ncols() == self.p_cov() {
            return Ok(gam_linalg::faer_ndarray::fast_atv(&deriv.x_psi, v));
        }
        let Some(op) = deriv.implicit_operator.as_ref() else {
            return Err(gam_terms::basis::BasisError::InvalidInput(format!(
    ...
    pub(crate) fn cov_forward_second(
        ...
        {
            return Ok(gam_linalg::faer_ndarray::fast_av(mat, u));
        }
    ...
    pub(crate) fn cov_transpose_second(
        ...
        if let Some(rows) = deriv_d.x_psi_psi.as_ref()
            ...
            && mat.ncols() == self.p_cov()
        {
            return Ok(gam_linalg::faer_ndarray::fast_atv(mat, v));
        }
    ...
    pub(crate) fn materialize_cov_first_axis_arc(
        &self,
        axis: usize,
    ) -> Result<Arc<Array2<f64>>, gam_terms::basis::BasisError> {
    ...
    pub(crate) fn lifted_forward(
        ...
        let beta = u
            ...
            .map_err(|_| {
                gam_terms::basis::BasisError::InvalidInput(
