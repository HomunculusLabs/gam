//! MEASUREMENT ONLY (#2579): prints the quantities the #561 regression guard
//! asserts but cannot reach, because it panics on assertion (1) (RMSE) before
//! evaluating EDF or the selected lambdas.
//!
//! The open question this settles: `multinomial_lambda_selection_561` fails at
//! RMSE=0.11641, and that scalar is consistent with EITHER direction — the
//! guard's own calibration says the fused-lambda driver measured >= 0.13 and
//! the pinned-lambda (dead-selection) driver >= 0.07. Meanwhile the penguins
//! arm (#2579) is measurably OVER-smoothed (collapsed to the base-rate model).
//! Over- and under-penalization are opposite defects that produce the same bad
//! RMSE, so the direction has to be read off EDF and lambda, not RMSE.
//!
//! Asserts only finiteness: this is an instrument, not a gate.
//!
//! Originally adapted from the #561 guard, which is:
...
#[test]
fn zz_measure_2579_multinomial_selection_direction() {
    let (ds, truth) = synth();
    let cfg = FitConfig::default();

    for init in [1.0_f64, 50.0_f64] {
        ...
        let edf = model.edf_per_class.clone();
        let lambdas = model.lambdas.clone();
        let echoes_seed = lambdas.iter().all(|&l| (l - init).abs() < 1e-6);
        let lam_min = lambdas.iter().cloned().fold(f64::MAX, f64::min);
        let lam_max = lambdas.iter().cloned().fold(f64::MIN, f64::max);

        println!("=== #2579 measure: init_lambda={init} ===");
        println!("  rmse_to_truth={rmse:.5}   (guard bar 0.065)");
        println!("  p_per_class={p_per_class:.1}");
        match edf.as_ref() {
            Some(e) => {
                for (a, &v) in e.iter().enumerate() {
                    // DIRECTION READOUT: edf_ratio near 1 means unpenalized
                    // (dead/fused selection); near 0 means smoothed to the null.
                    println!(
                        "  class {a}: edf={v:.4}  edf_ratio={:.4}",
                        v / p_per_class.max(1.0)
                    );
                }
            }
            None => println!("  edf_per_class=NONE"),
        }
        println!("  lambdas={lambdas:?}");
        println!("  lambda_min={lam_min:.6e} lambda_max={lam_max:.6e} spread={:.4}", lam_max / lam_min.max(f64::MIN_POSITIVE));
        println!("  echoes_seed={echoes_seed}");

        assert!(rmse.is_finite(), "rmse must be finite");
        assert!(
            lambdas.iter().all(|l| l.is_finite() && *l >= 0.0),
            "selected lambdas must be finite and non-negative: {lambdas:?}"
        );
    }
}
