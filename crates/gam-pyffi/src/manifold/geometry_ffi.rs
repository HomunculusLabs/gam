...
fn extend_model_with_group_impl(model_bytes: &[u8], request_json: &str) -> Result<Vec<u8>, String> {
    ...
    // layer's `prediction_model_class_label` reconstructs.
    ...
    let request: ExtendGroupRequest = serde_json::from_str(request_json)
        .map_err(|err| format!("failed to parse extend_with_group request: {err}"))?;
    // The mutation, its guards and the two save-time validations live in
...
fn predict_dataset_with_options_impl(
    ...
    serde_json::to_string(&PredictionPayload {
        ...
        // is only attached by the dedicated conformal predict entries.
