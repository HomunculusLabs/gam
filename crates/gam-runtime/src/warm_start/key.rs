...
///
/// Each `absorb_*` writes a per-type discriminator byte and the caller's content
/// tag before the data, so `absorb_str(b"x", "AB")` cannot collide with
/// `absorb_bytes(b"x", b"AB")`, nor `absorb_u64` with `absorb_bytes` of the same
/// little-endian bytes — heterogeneous fields sharing a tag can never alias.
