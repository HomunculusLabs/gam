//! Distribution-free coupling certificates for structure search.
//!
//! If `psi` is the per-row influence contribution for the pooled weighted
//! Pearson correlation `rho` between two gate streams, then any first-order
//! distribution shift with KL budget `epsilon` changes `rho` by at most
//! `sqrt(2 * epsilon * Var(psi))`. The conditional-coactivation influence values
//! carry the same first-order certificate for `P(active_j | active_i)`.
