...
impl SaeAtomBasisKind {
    ...
    /// Log of the cover sheets the ARD prior's normalizer must still divide out on
    /// this kind's cover chart (#2933 F25).
    ///
    /// Every deck group here is generated by one involution, so `|G| = 2`. The
    /// quotient density of a deck-invariant prior is `|G|` times its cover density,
    /// so a cover-normalized prior carries only `1/|G|` of its mass over one
    /// fundamental domain. A half-turned axis already absorbs that factor, because
    /// [`Self::ard_axis_periods`] normalizes its von Mises over the half period. A
    /// generator that only reflects (the ambient `RP²` antipode `u ~ -u`) leaves
    /// every factor's family and partition unchanged, so its quotient normalizer is
    /// the cover's less `log |G| = log 2` per row. The constant does not depend on
    /// the precisions, so no derivative channel sees it.
    pub(crate) fn ard_quotient_log_sheets(&self, latent_dim: usize) -> f64 {
        match self.deck_generator(latent_dim) {
            Some(generator) if !generator.contains(&DeckAxisAction::HalfTurn) => {
                std::f64::consts::LN_2
            }
            _ => 0.0,
        }
