//! Geometry-plan term builder, split out of `construction.rs` to keep that
//! tracked file under the #780 10k-line gate. `term_from_geometry_plans_with_mode`
//! assembles a [`SaeManifoldTerm`] from `(K, N, M_max[, D_max])`-padded arrays
//! validated against immutable geometry plans; it is re-exported from `mod.rs`
//! via `pub use construction_padded_blocks::*;` so every caller keeps reaching
...
///
/// Every active width, latent dimension, topology, evaluator, and
/// reference-metric identity is derived from `geometry_plans`. The padded arrays
