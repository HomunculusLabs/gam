...
impl SaeManifoldTerm {
    ...
    /// The deflation-folded per-row weight `E_tt` for the Trace row-jet seam
    /// (#2333, piece 1).
    ///
    /// The hand θ-adjoint loop computes its per-row contribution as
    /// `tr(inv_vv · D) − deflation_block_correction(inv_vv, D, dirs, spectrum)`:
    /// contract against the raw selected inverse, then subtract the
    /// Daleckii–Krein deflation correction. The `Trace` seam has no such
    /// two-step shape — it contracts a single symmetric weight against the
    /// materialized jet tower, `Σ_{a,b} E_tt[a,b]·dh[a,b]`. Routing the
    /// consumer through the seam therefore requires the SUBTRACTION to be
    /// folded into the WEIGHT, which is what this returns:
    ///
    /// ```text
    ///   Σ_{a,b} E[a,b]·D[a,b]  ==  tr(inv_vv·D) − deflation_block_correction(inv_vv, D, …)
    /// ```
    ///
    /// for every symmetric `D`. Pinned as an identity by
    /// `deflation_folded_e_tt_reproduces_the_trace_minus_correction_2333`.
    ///
    /// ## Why this is exactly `U (W ∘ F) Uᵀ`
    ///
    /// With `W = Uᵀ inv_vv U` and `M = Uᵀ D U`, orthogonality of `U` gives
    /// `tr(inv_vv·D) = Σ_{a,b} W[a,b]·M[a,b]`, while the correction is
    /// `Σ_{a,b} W[a,b]·M[a,b]·(1 − F[a,b])`. The difference is
    /// `Σ_{a,b} (W ∘ F)[a,b]·M[a,b] = tr(U(W ∘ F)Uᵀ · D)`. `W` is symmetric
    /// because `inv_vv` is, and `F` is symmetric because both its divided
    /// difference and its degenerate fallbacks are — so `E` is symmetric and is
    /// a legal seam weight. The SAME `F` (and hence the same gap threshold) is
    /// used as [`Self::deflation_block_correction`], via the shared
    /// [`Self::row_deflation_frechet_coefficients`], so the two cannot drift.
    ///
    /// ## The two non-spectral branches
    ///
    /// * gauge-only deflation (`spectrum = None`, non-empty `dirs`): the
    ///   correction is the within-row kept-subspace term `Σᵢ vᵢᵀ D vᵢ`, which is
    ///   `Σ_{a,b} (Σᵢ vᵢvᵢᵀ)[a,b]·D[a,b]`, so the fold is
    ///   `E = inv_vv − Σᵢ vᵢvᵢᵀ`. Short `vᵢ` are zero-extended exactly as the
    ///   correction does.
    /// * undeflated row, or a spectrum whose `evecs` are not `q×q` (the shape
    ///   guard on which the correction returns `0.0`): `E = inv_vv`.
    pub(crate) fn deflation_folded_e_tt(
        inv_vv: &Array2<f64>,
        dirs: &[Array1<f64>],
        spectrum: Option<&RowDeflationSpectrum>,
    ) -> Array2<f64> {
        ...
        let u = &spec.evecs;
        if u.nrows() != q || u.ncols() != q {
            // Same shape guard as `deflation_block_correction`, which yields a
            // zero correction here; the fold must then be the bare inverse.
            return inv_vv.clone();
        }
        let w = u.t().dot(inv_vv).dot(u);
        let f = Self::row_deflation_frechet_coefficients(spec, q);
        let mut folded = Array2::<f64>::zeros((q, q));
        for a in 0..q {
            for b in 0..q {
                folded[[a, b]] = w[[a, b]] * f[[a, b]];
            }
        }
