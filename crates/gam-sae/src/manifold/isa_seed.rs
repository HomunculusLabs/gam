...
//! its ambient support plane. Deflation is not a separation method here, and no
//! production path peels accepted planes. The multi-round deflationary producer,
//! which removes each accepted ambient plane's centered covariance projection
//! between capture rounds, lives in this file's tests, where it drives the #2111
//! producer gates.
//!
...
pub fn isa_eigen_parts(residual: ArrayView2<'_, f64>) -> Result<Option<IsaEigenParts>, String> {
    ...
    // (`isa_deflate_fitted_curve`, test scope) removes each accepted plane's two variance
...
mod tests {
    ...
    fn isa_deflate_fitted_curve(residual: &mut Array2<f64>, cand: &IsaPlaneCandidate) {
        let (n, p) = residual.dim();
        if n == 0 {
            return;
        }
    ...
    fn greedy_deflation_probe(
        ...
        while planes.len() < max_planes {
            ...
            let Some(cand) = isa_extract_certified_planes(work.view(), &parts, 1, config)
                .into_iter()
                .next()
            else {
                break;
