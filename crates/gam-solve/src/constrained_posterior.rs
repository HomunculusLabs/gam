...
mod projection_law_2446_tests {
    ...
    /// two-dimensional fixture and a scalar functional `f`, by tensor Simpson on
    /// the exact density. This is the reference: it never calls the cubature under
    /// test, and its own error is `O(h⁴)` at `h ≈ 3e-3`.
    ...
    /// `g` against the exact truncated density by the tensor Simpson rule of
    /// `exact_orthant_expectation_of`. Neither half calls the cubature under test.
    ///
