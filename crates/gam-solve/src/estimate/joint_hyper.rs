...
pub struct ExternalJointHyperEvaluator<'a> {
    ...
    /// [`crate::psi_gram_tensor::PsiGramTensor::reduced_basis_equal`].
    ...
    pub(crate) pending_psi_penalty:
        Option<std::sync::Arc<(Vec<gam_terms::construction::CanonicalPenalty>, Vec<usize>)>>,
...
impl<'a> ExternalJointHyperEvaluator<'a> {
    ...
    pub fn set_analytic_penalty_registry(
        ...
        let fingerprint = registry
            .map(crate::estimate::reml::outer_eval::analytic_penalty_registry_fingerprint)
            .unwrap_or(0);
        crate::estimate::reml::RemlState::set_analytic_penalty_registry_fingerprint(
            &mut self.reml_state,
            fingerprint,
        );
    }
    ...
    pub fn set_persistent_latent_values_fingerprint(
        ...
        let fingerprint =
            crate::estimate::reml::outer_eval::latent_id_mode_cache_fingerprint(id_mode);
        crate::estimate::reml::RemlState::set_persistent_latent_values_fingerprint(
            &mut self.reml_state,
            fingerprint,
        );
    }
    ...
    pub fn load_persistent_latent_values(
        ...
        crate::estimate::reml::RemlState::load_persistent_latent_values(
            &self.reml_state,
            n_obs,
            latent_dim,
        )
    ...
    pub fn build_and_set_psi_gram_tensor(
        ...
        let tensor = crate::psi_gram_tensor::PsiGramTensor::build(
            |psi| {
    ...
    pub fn build_frozen_glm_gram_tensor(
        ...
        psi_lo: f64,
        psi_hi: f64,
    ) -> Option<crate::glm_sufficient_lane::FrozenWeightGramTensor> {
        let conditioning = self.conditioning.clone();
        crate::glm_sufficient_lane::FrozenWeightGramTensor::build(
            |psi| {
                let raw = eval_raw_design(psi)?;
                Ok(conditioning.apply_to_design(&raw).to_dense())
            },
