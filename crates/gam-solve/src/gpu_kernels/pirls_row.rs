...
//
// The scalar evaluator the host replays to type a device refusal, and that the
// Stage 1 parity tests compare the GPU buffers to, lives in
// `pirls_row_cpu_reference.rs`. Every consumer is Linux-only.
// ────────────────────────────────────────────────────────────────────────
...
#[cfg(target_os = "linux")]
#[path = "pirls_row_cpu_reference.rs"]
mod cpu_reference;
#[cfg(target_os = "linux")]
pub(crate) use cpu_reference::{RowInput, replay_first_refusal, row_reweight_cpu_at};
