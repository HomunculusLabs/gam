use crate::estimate::{EstimationError, UnifiedFitResult, dispersion_from_likelihood};
use crate::pirls;
...
impl<'a> AloInput<'a> {
    ...
    ///
    /// The constructor is exact only when the score- and Hessian-side IRLS
    /// weights coincide (the canonical-link case, Fisher == Observed): the
    /// supplied `working_weights` are the score-side Fisher weights
    /// `h'²/(φ V(μ)) ≥ 0`, so they are PSD by construction and are re-viewed for
    /// the Hessian-side slot via `as_signed()`.
    ///
