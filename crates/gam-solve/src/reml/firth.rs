...
impl FirthDenseOperator {
    ...
    pub(crate) fn jeffreys_beta_gradient(&self) -> Array1<f64> {
        ...
        0.5 * gam_linalg::faer_ndarray::fast_av(&self.x_dense_t, &(&self.w1 * &self.h_diag))
    }
    ...
    pub(crate) fn dot_i_and_h_from_reduced(
        ...
        let dot_i = RemlState::weighted_cross(x_tau_reduced, &self.x_reduced, &self.w)
            ...
            + gam_linalg::faer_ndarray::fast_xt_diag_x(&self.x_reduced, &dw);
    ...
    pub(crate) fn exact_tau_kernel(
        ...
        // subtraction.
        let deta_partial = gam_linalg::faer_ndarray::fast_av(x_tau, beta);
        ...
        let first = 0.5 * gam_linalg::faer_ndarray::fast_atv(x_tau, &(&self.w1 * &self.h_diag));
        let secondvec =
            &(&(&self.w2 * &deta_partial) * &self.h_diag) + &(&self.w1 * &dot_h_partial);
        let second = 0.5 * gam_linalg::faer_ndarray::fast_atv(&self.x_dense, &secondvec);
    ...
    pub(crate) fn hphi_tau_tau_partial_apply(
        ...
        i_ddot = i_ddot + gam_linalg::faer_ndarray::fast_xt_diag_x(x_r, &ddw_ij);
...
mod tests {
    ...
    pub(crate) fn rank_deficient_and_explicit_reduced_designs_share_same_jeffreys_objective() {
        ...
        let op_full = build_weighted_logit_firth_dense_operator(
            &x_full,
            &eta_full,
            observation_weights.view(),
        )
        .expect("full firth operator");
        let op_reduced = build_weighted_logit_firth_dense_operator(
            &x_reduced,
            &eta_reduced,
            observation_weights.view(),
        )
    ...
    pub(crate) fn firthphi_tau_tau_pair_scalar_matches_finite_difference() {
        ...
        let eval_phi_tau_i = |x_eval: &Array2<f64>| -> f64 {
            ...
            let op_e = build_logit_firth_dense_operator(x_eval, &eta_e).expect("perturbed op");
