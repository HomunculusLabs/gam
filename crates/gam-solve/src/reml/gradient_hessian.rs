...
impl<'a> RemlState<'a> {
    ...
    pub(crate) fn tierney_kadane_terms(
        ...
        // These penalties are consumed below as `A_k β̂ = λ_k S_k β̂` and
        // `tr(λ_k S_k P)` — both of them `∂H/∂ρ_k` objects. `H` contains the
        // split-projected `S̃(λ) = E(λ)ᵀE(λ)`, so the penalty spelled here has
        // to be `λ_k Π S_k Π` for exactly the reason it does in the outer
        // ρ-gradient (#2454): the difference between the two is β̂'s energy in
        // the directions the λ-invariant split declares null, multiplied by
        // `λ_k` — an additive `c·λ` that is invisible at `‖ρ‖ ≤ 1` and grows
        // without bound above it.
        //
        // `project_canonical` returns the penalty untouched when the projection
        // is below the roots' own representation noise, which is the ordinary
