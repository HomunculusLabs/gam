...
pub(crate) struct TauTauPairHyperOperator {
    ...
    // carried as the owned [`gam_linalg::matrix::SignedWeightsArc`] newtype so the
...
impl super::reml_outer_engine::HyperOperator for TauTauPairHyperOperator {
    ...
    fn mul_vec(&self, v: &Array1<f64>) -> Array1<f64> {
        ...
        let tau_transpose = |term: &TauDesignTerm, y: &Array1<f64>| -> Array1<f64> {
            match term {
                TauDesignTerm::Dense(dense) => gam_linalg::faer_ndarray::fast_atv(dense, y),
                TauDesignTerm::Implicit(deriv) => match &self.basis {
                    TauPairBasis::Original => deriv
        ...
        let tau_tau_transpose = |term: &TauTauDesignTerm, y: &Array1<f64>| -> Array1<f64> {
            match term {
                TauTauDesignTerm::Dense(dense) => gam_linalg::faer_ndarray::fast_atv(dense, y),
                TauTauDesignTerm::Implicit(deriv) => match &self.basis {
                    TauPairBasis::Original => deriv
                        .transpose_mul_original(y)
...
impl<'a> RemlState<'a> {
    ...
    pub(crate) fn tau_design_transpose_mul(
        ...
        match term {
            TauDesignTerm::Dense(dense) => {
                ...
                Ok(gam_linalg::faer_ndarray::fast_atv(dense, v))
            }
    ...
    {
        Box::new(
            move |k: usize, j: usize| -> super::reml_outer_engine::HyperCoordPairResult {
    ...
    pub(in crate::estimate) fn compute_joint_hyper_eval_with_order(
        &self,
        theta: &Array1<f64>,
        rho_dim: usize,
        hyper_dirs: &[DirectionalHyperParam],
    ...
    > {
        ...
        if p_dim == 0 {
            let tau_tau_pair_fn =
                move |_: usize, _: usize| -> super::reml_outer_engine::HyperCoordPairResult {
                    Ok(super::reml_outer_engine::HyperCoordPair {
                        a: 0.0,
