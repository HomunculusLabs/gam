...
//!      (gam#2612).
//!   3. **The MEASURED span** ([`under_identified_subspace_in_metric`]): the directions
...
fn symmetrize_contiguous(matrix: &mut Array2<f64>) {
    ...
    assert_eq!(
        rows, columns,
        "spectral symmetrization requires a square matrix"
    );
    let values = matrix
        .as_slice_mut()
...
/// `∂²g/∂floor²` with `λ` held fixed, on the same smooth branch as
/// [`jeffreys_antiderivative_floor_sensitivity`].  This is the missing
/// moving-relative-floor channel in second-order Jeffreys value calculus:
/// when `floor = REDUCED_INFO_RELATIVE_FLOOR · λ_max`, differentiating the
...
pub(crate) fn jeffreys_antiderivative_floor_second_sensitivity(lam: f64, floor: f64) -> f64 {
    ...
    if lam >= cap {
        if cap > CONDITIONING_GATE_ABSOLUTE_CLEAR {
            // Floor-bound top saturation:
            // g = ln(floor) + 1 - floor/λ.
            -1.0 / (floor * floor)
        } else {
            // Gate-bound top saturation does not move with the floor.
            0.0
        }
    } else if lam >= floor {
        0.0
...
pub(crate) fn floored_inverse_divided_differences(evals: &Array1<f64>, floor: f64) -> Array2<f64> {
    ...
    for i in 0..m {
        ...
        for j in 0..m {
            ...
            values[i * m + j] = mixed::inverse_difference(&[lambda_i, lambda_j], floor, 0);
        }
    }
    psi
}
...
/// `d/dx = −6t(1−t)/span`, `d²/dx² = −6(1−2t)/span²`, `d³/dx³ = 12/span³`. Every
/// higher derivative of the cubic is zero.
#[inline]
fn conditioning_ramp_down_derivatives(x: f64, under: f64, clear: f64) -> (f64, f64, f64, f64) {
    if x <= under || x >= clear {
...
/// derivative of the gated Jeffreys value.
///
/// For `P = ∂_s H_info|_β`, a coefficient-space direction `v`,
/// `D = D_β H_info[v]`, and `E = D_β(∂_s H_info)[v]`, the exact
/// fixed-stratum identity is
///
/// ```text
/// ∂²_{s,β[v]} Φ = <beta_information, D>_F + <mixed_information, E>_F.
/// ```
///
/// `mixed_information` is `∇_H Φ`; `beta_information` is its Fréchet
/// derivative along `P`.  The pair is prepared from one authoritative
/// [`JointJeffreysPlan`] spectrum and includes conditioning-gate motion and the
/// motion of the relative floor `floor = REL · λ_max`.
#[derive(Clone, Debug)]
pub struct JointJeffreysExplicitMixedTraceWeights {
    /// Weight contracted with `D_β H_info[v]`.
    pub beta_information: Array2<f64>,
    /// Weight contracted with `D_β(∂_s H_info)[v]`.
    pub mixed_information: Array2<f64>,
}

impl JointJeffreysExplicitMixedTraceWeights {
    /// Contract the prepared weights with the two family-provided information
    /// derivatives.  Both weights are symmetric, so any antisymmetric numerical
    /// residue in an input is ignored automatically by the Frobenius product.
    pub fn contract(
        &self,
        beta_information_derivative: &Array2<f64>,
        mixed_information_derivative: &Array2<f64>,
    ) -> Result<f64, String> {
        let shape = self.beta_information.dim();
        if beta_information_derivative.dim() != shape {
            return Err(format!(
                "joint Jeffreys explicit mixed contraction: beta-information derivative shape {:?} != {:?}",
                beta_information_derivative.dim(),
                shape
            ));
        }
        if mixed_information_derivative.dim() != shape {
            return Err(format!(
                "joint Jeffreys explicit mixed contraction: mixed-information derivative shape {:?} != {:?}",
                mixed_information_derivative.dim(),
                shape
            ));
        }
        let beta_term = self
            .beta_information
            .iter()
            .zip(beta_information_derivative.iter())
            .map(|(&weight, &value)| weight * value)
            .sum::<f64>();
        let mixed_term = self
            .mixed_information
            .iter()
            .zip(mixed_information_derivative.iter())
            .map(|(&weight, &value)| weight * value)
            .sum::<f64>();
...
impl JointJeffreysPlan {
    ...
    /// `P = ∂_s H_info|_β`.
    ///
    /// This differentiates the complete spectral scalar
    ///
    /// ```text
    /// Φ(H) = G(λ_min, λ_max) · ½ Σ_i g(λ_i, floor),
    /// floor = max(REL · λ_max, ABS),
    /// ```
    ///
    /// rather than differentiating the floored inverse while holding `floor`
    /// fixed.  It therefore contains `g_{λ,floor}`, `g_{floor,floor}`, gate
    /// gradient, and gate Hessian channels.
    ///
    /// The Hessian of this C¹ spectral function is defined only on a fixed C²
    /// stratum: the reduced spectrum must be simple, no eigenvalue may sit on an
    /// antiderivative branch knot, and the floor, cap, and gate policies must be
    /// off their ramp/max branch boundaries.  Those are real model-stratum
    /// boundaries, not numerical singularities, so this method fails closed
    /// instead of selecting an arbitrary second derivative there.
    pub fn explicit_param_mixed_trace_weights(
        &self,
        explicit_information_derivative: &Array2<f64>,
    ) -> Result<JointJeffreysExplicitMixedTraceWeights, String> {
        let p = self.coefficient_dim();
        if explicit_information_derivative.dim() != (p, p) {
            return Err(format!(
                "joint Jeffreys explicit mixed trace weights: perturbation shape {:?} != ({p}, {p})",
                explicit_information_derivative.dim()
            ));
        }
        let m = self.reduced_dim;
        if m == 0 {
            return Ok(JointJeffreysExplicitMixedTraceWeights {
                beta_information: Array2::zeros((p, p)),
                mixed_information: Array2::zeros((p, p)),
            });
        }

        let spectral_scale = self
            .evals
            .iter()
            .map(|value| value.abs())
            .fold(1.0_f64, f64::max);
        let branch_tol = 64.0 * f64::EPSILON * spectral_scale;

        // The cubic ramps are C¹ rather than C² at their endpoints, and
        ...
        if self.lambda_max > 0.0 && self.lambda_min.is_finite() {
            for ratio_knot in [CONDITIONING_GATE_RELATIVE, CONDITIONING_GATE_RELATIVE_CLEAR] {
                ...
                if signed_distance.abs() <= branch_tol {
                    return Err(format!(
                        "joint Jeffreys explicit mixed trace weights reached a relative-gate stratum: λ_min-({ratio_knot:e})·λ_max={signed_distance:e}, tolerance={branch_tol:e}"
                    ));
                }
            }
            ...
            {
                return Err(format!(
                    "joint Jeffreys explicit mixed trace weights reached the max-gate branch stratum: w_abs={weight_absolute:e}, w_rel={weight_relative:e}"
                ));
            }
        }
        if self.gate_weight == 0.0 {
            // The conditioning gate is locally constant on its inactive
            // plateau, so the value, gradient, and Hessian are exactly zero.
            // Return that exact artifact before inspecting any floor/cap or
            // spectral stratum: those policies are immaterial to an absent
            // term. Gate boundaries themselves were checked above because the
            // zero plateau is not a C2 neighborhood at those points.
            return Ok(JointJeffreysExplicitMixedTraceWeights {
                beta_information: Array2::zeros((p, p)),
                mixed_information: Array2::zeros((p, p)),
            });
        ...
        for (index, &lambda) in self.evals.iter().enumerate() {
            ...
            {
                return Err(format!(
                    "joint Jeffreys explicit mixed trace weights reached a non-C2 antiderivative stratum at λ[{index}]={lambda:e} (floor={:e}, cap={cap:e}, tolerance={branch_tol:e})",
                    self.floor
                ));
            }
        }
        ...
        if (relative_floor - REDUCED_INFO_ABSOLUTE_FLOOR).abs() <= floor_knot_tol {
            return Err(format!(
                ...
                REDUCED_INFO_ABSOLUTE_FLOOR
            ));
        }

        // Reduce and rotate the explicit perturbation into the authoritative
        // eigenbasis.  Symmetrization matches the information value path and
        ...
        let mut value_ungated = 0.0_f64;
        let mut floor_first_sum = 0.0_f64;
        let mut floor_second_sum = 0.0_f64;
        let mut d = Array1::<f64>::zeros(m);
        let mut d_lambda = Array1::<f64>::zeros(m);
        let mut d_floor = Array1::<f64>::zeros(m);
        for i in 0..m {
            let lambda = self.evals[i];
            ...
            d_lambda[i] = floored_inverse_prime(lambda, self.floor);
            d_floor[i] = floored_inverse_floor_sensitivity(lambda, self.floor);
        }
        value_ungated *= 0.5;
        ...
        let gate_gradient_values = gate_gradient
            .as_slice()
            .expect("gate gradient is contiguous");
        let gate_hessian_values = gate_hessian.as_slice().expect("gate Hessian is contiguous");
        let b_values = b
            .as_slice_mut()
        ...
        for i in 0..m {
            b_values[i] = self.gate_weight * u_values[i] + value_ungated * gate_gradient_values[i];
            for j in 0..m {
                let index = i * m + j;
        ...
        // Fréchet derivative of the spectral gradient along P.  Diagonal
        // entries use the eigenvalue Hessian; off-diagonal entries are the
        // exact divided differences of the complete eigen-gradient `b`, which
        ...
        let extreme_active = |i| {
            (i == self.idx_min && (gate_grad_min != 0.0 || gate_hess_min_min != 0.0 || gate_hess_min_max != 0.0))
                || (i == self.idx_max && (gate_grad_max != 0.0 || gate_hess_max_max != 0.0 || gate_hess_min_max != 0.0
                    || (floor_rate != 0.0 && (floor_first_sum != 0.0 || floor_second_sum != 0.0))))
        };
        let extra_gradient = |i| {
            value_ungated * gate_gradient_values[i]
                + if i == self.idx_max { 0.5 * self.gate_weight * floor_rate * floor_first_sum } else { 0.0 }
        };
        for i in 0..m {
            ...
            for j in (i + 1)..m {
                ...
                let active = extreme_active(i) || extreme_active(j);
                if active && gap.abs() <= 16.0 * f64::EPSILON * self.evals[i].abs().max(self.evals[j].abs()) {
                    return Err("joint Jeffreys explicit mixed trace weights require a simple active extreme eigenvalue".into());
                }
                // The ordinary spectral part has an exact rational divided
                // difference, including at repeated interior eigenvalues.
                // Only active gate/floor projectors require an eigengap.
                let mut ratio = 0.5 * self.gate_weight
                    * mixed::inverse_difference(&[self.evals[i], self.evals[j]], self.floor, 0);
                if active { ratio += (extra_gradient(i) - extra_gradient(j)) / gap; }
        ...
        {
            return Err(
                "joint Jeffreys explicit mixed trace weights produced non-finite ambient weights"
                    .to_string(),
            );
        }
        Ok(JointJeffreysExplicitMixedTraceWeights {
            beta_information,
            mixed_information,
        })
    }
...
///
/// Inputs are the snapshot `H_info`, span `Z_J`, first information derivatives
/// `pert_i = ∂_{s_i}H_info|_β` and `pert_j = ∂_{s_j}H_info|_β`, and the mixed
/// derivative `pert_ij = ∂²_{s_i s_j}H_info|_β`.  The implementation prepares
/// [`JointJeffreysExplicitMixedTraceWeights`] from `pert_i`, then contracts its
/// `(A, B)` weights with `(pert_j, pert_ij)`.  This includes the full gate
/// Hessian and the first and second motion of the relative floor
/// `floor = REDUCED_INFO_RELATIVE_FLOOR · λ_max`; it does not freeze either
/// moving spectral policy.  Non-C² spectral-stratum boundaries fail closed.
pub fn joint_jeffreys_phi_explicit_param_second_derivative(
    ...
    let plan = JointJeffreysPlan::prepare(h_joint, z_j)?;
    let weights = plan.explicit_param_mixed_trace_weights(pert_i)?;
    weights.contract(pert_j, pert_ij)
}
...
impl JeffreysHphiDriftBase {
    ...
    fn motion_drift_from_rows(
        ...
        let d3_extreme = |e: usize, factors: [&Array2<f64>; 3]| -> f64 {
            simple_eigenvalue_third_form(evals, tie_tolerance, e, factors)
        };
...
/// `1/(λ_e − λ_j)` on the reduced resolvent of a simple eigenvalue: zero on the
/// eigenvalue itself and within `tie_tolerance` of a tie, where the perturbation
/// series has no term.
#[inline]
fn simple_eigenvalue_gap_inverse(evals: &Array1<f64>, tie_tolerance: f64, e: usize, j: usize) -> f64 {
...
/// Second symmetric form `D²λ_e[X, Y] = 2 Σ_{j≠e} X[e,j]·Y[e,j] / (λ_e − λ_j)` of a
/// simple eigenvalue, with the factors written in its eigenbasis.
fn simple_eigenvalue_second_form(
    evals: &Array1<f64>,
    tie_tolerance: f64,
    e: usize,
    x: &Array2<f64>,
    ...
    for j in 0..evals.len() {
        let inv = simple_eigenvalue_gap_inverse(evals, tie_tolerance, e, j);
        if inv != 0.0 {
            sum += x[[e, j]] * y[[e, j]] * inv;
        }
    }
...
/// Third symmetric form of a simple eigenvalue,
/// `D³λ_e[X, Y, Z] = Σ_{σ∈S₃} ( x̂_σ₁ᵀ Y_σ₂ x̂_σ₃ − X_σ₁[e,e]·(x̂_σ₂·x̂_σ₃) )`,
/// `x̂[j] = X[e,j]/(λ_e − λ_j)`.
fn simple_eigenvalue_third_form(
    evals: &Array1<f64>,
    tie_tolerance: f64,
    e: usize,
    ...
    let hats: Vec<Array1<f64>> = factors
        ...
        .map(|x| {
            Array1::from_shape_fn(m, |j| {
                x[[e, j]] * simple_eigenvalue_gap_inverse(evals, tie_tolerance, e, j)
            })
        })
        .collect();
    let mut sum = 0.0_f64;
            ...
            a_rows
                ...
                .for_each(|((mut a_row, mut aw_row), hdot_a)| {
                    gam_problem::with_nested_parallel(|| {
                        let a_a =
                            symmetric_basis_contraction(hdot_a.view(), ambient_eigenbasis.view());
                        let a_values = a_a
                            .as_slice()
...
mod tests {
    ...
    /// gam#2894: the frozen-policy second completion drift `D_w(D completion[u]·v)` against
    /// central differences of the frozen first drift along `w`, with the drift base
    /// re-prepared at every perturbed β. Returns `(max |analytic|, relative error)`.
    fn jeffreys_frozen_completion_second_drift_error_2894<I, F, S, T, Q>(
        beta: &Array1<f64>,
        information: I,
        first: F,
        second: S,
    ...
    where
        ...
        S: Fn(&Array1<f64>, &Array1<f64>, &Array1<f64>) -> Array2<f64>,
        T: Fn(&Array1<f64>, &Array1<f64>, &Array1<f64>, &Array1<f64>) -> Array2<f64>,
        Q: Fn(&Array1<f64>, &Array1<f64>, &Array1<f64>, &Array1<f64>, &Array1<f64>) -> Array2<f64>,
    {
        let p = beta.len();
        let z = Array2::<f64>::eye(p);
        let axis = |a: usize| {
        ...
        let frozen_drift_at = |b: &Array1<f64>| {
            let axes_v: Vec<Array2<f64>> = (0..p).map(|a| second(b, &v, &axis(a))).collect();
            let moving: Vec<Array2<f64>> = (0..p).map(|a| third(b, &u, &v, &axis(a))).collect();
            base_at(b)
                .completion_drift_action(&first(b, &u), &axes_v, &moving)
                .expect("frozen completion drift")
        };
        ...
        let analytic = base
            ...
            .expect("frozen second completion drift");
        let step = 1e-5;
        let fd = (frozen_drift_at(&(beta + &(&w * step))) - frozen_drift_at(&(beta - &(&w * step))))
            / (2.0 * step);
        let max_abs = |x: &Array1<f64>| x.iter().fold(0.0_f64, |acc, value| acc.max(value.abs()));
        let scale = max_abs(&fd).max(1e-12);
        (max_abs(&analytic), max_abs(&(&analytic - &fd)) / scale)
    }

    /// gam#2894: inside the gate's transition band the frozen second drift carries the
    /// gate's second partials and the second Fréchet derivative of the capped inverse.
    #[test]
    fn jeffreys_frozen_completion_second_drift_matches_fd_in_the_gate_band_2894() {
        let rows = array![
            [1.0, 0.2, -0.3],
            [1.0, -0.5, 0.4],
            [1.0, 0.9, 0.1],
        ...
        let lambda_min = evals.iter().copied().fold(f64::INFINITY, f64::min);
        let scale = 4.0 / lambda_min;
        let (magnitude, error) = jeffreys_frozen_completion_second_drift_error_2894(
            &beta,
            |b| raw(b, &[]).mapv(|value| scale * value),
            |b, d| raw(b, &[d]).mapv(|value| scale * value),
            |b, x, y| raw(b, &[x, y]).mapv(|value| scale * value),
            |b, x, y, w| raw(b, &[x, y, w]).mapv(|value| scale * value),
            |b, x, y, w, t| raw(b, &[x, y, w, t]).mapv(|value| scale * value),
        );
        assert!(magnitude > 1e-8, "the fixture must exercise a nonzero second drift");
        assert!(
            error < 1e-5,
            "frozen second completion drift vs finite differences: rel {error:e}"
        );
    }

    /// gam#2894: under a moving relative floor the frozen second drift carries the
    /// floor's second channels.
    #[test]
    fn jeffreys_frozen_completion_second_drift_matches_fd_under_a_moving_floor_2894() {
        let rows = array![[1.0, 0.3], [1.0, -0.6], [1.0, 0.8], [1.0, -0.2], [1.0, 0.5]];
        let beta = array![0.2, -0.1, 0.4];
        let ghost = 1e-13;
        let raw = |b: &Array1<f64>, directions: &[&Array1<f64>]| {
    ...
    /// gam#2894: every column of the completion drift matrix equals the motion-completed
    /// completion drift action along that axis. Both sides are exact algebra on the same
    /// information derivatives, so they agree to roundoff. Returns
    /// `(motion active, max |matrix|, relative error)`.
    fn jeffreys_completion_drift_matrix_errors_2894<I, F, S, T>(
        beta: &Array1<f64>,
        information: I,
        first: F,
        second: S,
        ...
        T: Fn(&Array1<f64>, &Array1<f64>, &Array1<f64>, &Array1<f64>) -> Array2<f64>,
    {
        ...
        let base = JeffreysHphiDriftBase::prepare_with_axes(information(beta).view(), z.view(), hdots)
            .expect("drift base")
            .expect("active drift base");
        let pert_u = first(beta, &u);
        let second_u = base
            .rotate_axes(&(0..p).map(|a| second(beta, &u, &axis(a))).collect::<Vec<_>>())
            .expect("rotated H²[u,·]");
        ...
        let contracted =
            |weight: &Array2<f64>| contract(weight, &|a, b| second(beta, &axis(a), &axis(b)));
        ...
        (base.hessian_motion_active(), largest, worst / largest.max(1e-300))
    }

    /// gam#2894: inside the gate's transition band the completion drift matrix carries the
    /// gate motion's weights and remainder drift, column for column.
    #[test]
    fn jeffreys_completion_drift_matrix_matches_the_drift_action_in_the_gate_band_2894() {
        let rows = array![
            [1.0, 0.2, -0.3],
            [1.0, -0.5, 0.4],
            [1.0, 0.9, 0.1],
        ...
        let lambda_min = evals.iter().copied().fold(f64::INFINITY, f64::min);
        let scale = 4.0 / lambda_min;
        let (active, magnitude, error) = jeffreys_completion_drift_matrix_errors_2894(
            &beta,
            |b| raw(b, &[]).mapv(|value| scale * value),
            |b, d| raw(b, &[d]).mapv(|value| scale * value),
            |b, x, y| raw(b, &[x, y]).mapv(|value| scale * value),
        ...
        assert!(active, "the fixture must sit inside the gate's transition band");
        assert!(magnitude > 1e-8, "the fixture must exercise a nonzero completion drift");
        assert!(error < 1e-8, "completion drift matrix vs drift action: rel {error:e}");
    }

    /// gam#2894: under a moving relative floor the completion drift matrix carries the floor
    /// motion's channels, column for column.
    #[test]
    fn jeffreys_completion_drift_matrix_matches_the_drift_action_under_a_moving_floor_2894() {
        let rows = array![[1.0, 0.3], [1.0, -0.6], [1.0, 0.8], [1.0, -0.2], [1.0, 0.5]];
        let beta = array![0.2, -0.1, 0.4];
        let ghost = 1e-13;
        let raw = |b: &Array1<f64>, directions: &[&Array1<f64>]| {
    ...
    fn fused_ambient_eigenbasis_congruence_matches_two_stage_definition_2612() {
        ...
        let v = array![[0.8, -0.6, 0.0], [0.6, 0.8, 0.0], [0.0, 0.0, 1.0],];

        let reduced_raw = z.t().dot(&matrix.dot(&z));
        let sequential = v
            .t()
            .dot(&((&reduced_raw + &reduced_raw.t()).mapv(|value| 0.5 * value)))
            .dot(&v);
        let ambient_eigenbasis = z.dot(&v);
        let fused = symmetric_basis_contraction(matrix.view(), ambient_eigenbasis.view());
    ...
    #[test]
    pub(crate) fn joint_jeffreys_phi_explicit_param_derivatives_match_finite_difference() {
        // EXPLICIT-parameter (β-fixed) first AND second derivatives of the gated
        // Jeffreys VALUE Φ w.r.t. a scalar `s` entering H_info as H_info(s)=H0+s·P+½s²·Q.
        // References are central FD / second-difference of `joint_jeffreys_term(...).0`.
        // H0's spectrum places λ_min in the conditioning-gate band ([1,16]) so the gate
        // motion is exercised; P, Q are non-diagonal so the eigenbasis rotation matters.
        let p = 3usize;
        let z = Array2::<f64>::eye(p);
        let q = {
            let a = array![[1.0, 0.3, -0.2], [0.3, 1.0, 0.4], [-0.2, 0.4, 1.0]];
            let (_e, v) = a.eigh(Side::Lower).unwrap();
            v
        };
        let h0 = {
            let d = Array2::from_diag(&array![2.0, 8.0, 30.0]);
            q.dot(&d).dot(&q.t())
        };
        let pert = array![[0.7, -0.25, 0.1], [-0.25, 1.3, 0.35], [0.1, 0.35, -0.6]];
        ...
        assert!(
            (an1 - fd1).abs() < 1e-5,
            "explicit Φ first derivative {an1} vs FD {fd1}"
        );
        assert!(
            (an2 - fd2).abs() < 1e-3,
            "explicit Φ second derivative {an2} vs FD {fd2}"
        );
    }

    #[test]
    pub(crate) fn joint_jeffreys_explicit_mixed_weights_include_relative_floor_motion() {
        // Smallest witness for the moving-floor channel.  The first eigenvalue
        // is below `floor = REL · λ_max`, while the second eigenvalue moves.
        // Holding the floor fixed makes <A,D> spuriously zero; differentiating
        // the value actually gives -1/4.  A nonzero mixed information derivative
        // additionally exercises the independent <B,E> contraction.
        let z = Array2::<f64>::eye(2);
        let h0 = array![[5.0e-11, 0.0], [0.0, 1.0]];
        let explicit = array![[5.0e-11, 0.0], [0.0, 0.0]];
        let beta_derivative = array![[0.0, 0.0], [0.0, 1.0]];
        ...
        let contracted = weights
            .contract(&beta_derivative, &mixed_derivative)
            .unwrap();
        ...
        .unwrap();
        assert!(
            (contracted - (a_dot_d + b_dot_e)).abs() < 1.0e-14,
            "prepared A/B contraction {contracted} vs explicit sum {}",
            a_dot_d + b_dot_e
        );
        assert!(
            (scalar - contracted).abs() < 1.0e-14,
            "scalar mixed derivative {scalar} vs prepared contraction {contracted}"
        );
    ...
    #[test]
    pub(crate) fn conditioning_gate_weight_fourth_matches_finite_difference_2894() {
        // The five fourth partials against central differences of the (FD-validated)
        // third partials, over the configs of the third-partial check.
        let configs: [(f64, f64); 8] = [
            (8.0, 1.0e9),
            (4.0, 1.0e9),
            (12.0, 1.0e9),
        ...
        for &(lmin, lmax) in &configs {
            ...
            if [q_mmmm, q_mmmx, q_mmxx, q_mxxx, q_xxxx].iter().any(|v| *v != 0.0) {
                nonzero_configs += 1;
            }
            let hmin = 1e-6 * lmin.abs().max(1e-3);
            let hmax = 1e-6 * lmax.abs().max(1e-3);
            let d_min = |pick: fn((f64, f64, f64, f64)) -> f64| {
                (pick(conditioning_gate_weight_third(lmin + hmin, lmax))
                    - pick(conditioning_gate_weight_third(lmin - hmin, lmax)))
                    / (2.0 * hmin)
            };
            let d_max = |pick: fn((f64, f64, f64, f64)) -> f64| {
                (pick(conditioning_gate_weight_third(lmin, lmax + hmax))
                    - pick(conditioning_gate_weight_third(lmin, lmax - hmax)))
                    / (2.0 * hmax)
            };
            let checks = [
                ("G1111", q_mmmm, d_min(|t| t.0)),
                ("G1112", q_mmmx, d_max(|t| t.0)),
                ("G1122", q_mmxx, d_max(|t| t.1)),
                ("G1222", q_mxxx, d_max(|t| t.2)),
                ("G2222", q_xxxx, d_max(|t| t.3)),
            ];
            for (label, analytic, fd) in checks {
                assert!(
                    (fd - analytic).abs() <= 1e-3 * analytic.abs().max(1.0),
        ...
        // Only the relative band carries a fourth partial: the absolute ramp is cubic.
        assert!(
            nonzero_configs >= 2,
    ...
    #[test]
    pub(crate) fn simple_eigenvalue_fourth_form_matches_fd_of_third_2894() {
        // `A(t) = A0 + t·X4` is linear, so the derivative of the third form along X4 is
        // exactly the pure fourth form: no higher information derivative enters. The
        // third form is taken in the perturbed eigenbasis, for the smallest and the
        // largest eigenvalue.
        fn argmin(values: &Array1<f64>) -> usize {
            (0..values.len())
        ...
        let symmetric = |raw: Array2<f64>| (&raw + &raw.t()) * 0.5;
        let a0 = array![
            [2.0, 0.3, -0.1, 0.05],
            [0.3, 3.5, 0.2, -0.15],
            [-0.1, 0.2, 5.0, 0.25],
        ...
        let x1 = symmetric(array![
            [0.4, -0.2, 0.1, 0.3],
            [0.1, 0.25, -0.3, 0.05],
            [0.2, -0.1, -0.35, 0.15],
            [-0.05, 0.2, 0.1, 0.5]
        ]);
        let x2 = symmetric(array![
            [-0.3, 0.15, 0.2, -0.1],
            [0.25, 0.4, 0.05, 0.2],
            [-0.15, 0.3, 0.1, -0.25],
            [0.1, -0.05, 0.35, -0.2]
        ]);
        let x3 = symmetric(array![
            [0.2, 0.35, -0.25, 0.1],
            [-0.1, -0.15, 0.3, 0.2],
            [0.3, 0.05, 0.45, -0.1],
            [0.15, 0.25, -0.2, 0.3]
        ]);
        let x4 = symmetric(array![
            [0.1, -0.3, 0.2, 0.25],
            [0.2, -0.25, 0.15, -0.1],
            [0.05, 0.35, -0.2, 0.3],
            [-0.2, 0.1, 0.25, 0.15]
        ]);
        let spectral_frame = |matrix: &Array2<f64>, pick: fn(&Array1<f64>) -> usize| {
            let (evals, evecs) = matrix.eigh(Side::Lower).expect("symmetric spectrum");
            let e = pick(&evals);
            let scale = evals.iter().fold(1.0_f64, |acc, value| acc.max(value.abs()));
            let tie_tolerance = 64.0 * f64::EPSILON * scale;
            let rotate = |x: &Array2<f64>| evecs.t().dot(x).dot(&evecs);
            (evals, e, tie_tolerance, [rotate(&x1), rotate(&x2), rotate(&x3), rotate(&x4)])
        };
        for (label, pick) in [("min", argmin as fn(&Array1<f64>) -> usize), ("max", argmax)] {
            let (evals, e, tie_tolerance, rotated) = spectral_frame(&a0, pick);
            let fourth = simple_eigenvalue_fourth_form(
                &evals,
                tie_tolerance,
                e,
            ...
            let step = 1.0e-5;
            let third_at = |sign: f64| {
                let (evals, e, tie_tolerance, rotated) =
                    spectral_frame(&(&a0 + &(&x4 * (sign * step))), pick);
                simple_eigenvalue_third_form(
                    &evals,
                    tie_tolerance,
                    e,
            ...
            let fd = (third_at(1.0) - third_at(-1.0)) / (2.0 * step);
            assert!(
                (fourth - fd).abs() <= 1.0e-6 * (1.0 + fd.abs()),
                "{label}: fourth form={fourth:e} fd of third={fd:e}"
            );
            assert!(
                fourth.abs() > 1.0e-4,
                "{label}: the fixture must exercise a nonzero fourth form, got {fourth:e}"
            );
        }
    }

    #[test]
    pub(crate) fn floor_sensitivities_match_finite_difference_2894() {
        // The four floor channels the second completion drift reads, against central
        // differences of the (FD-validated) lower orders on every branch of the capped
        // inverse: a floor-bound cap, a gate-bound cap, the log window, the floor plateau
        // and the saturating negative branch.
        let points: [(f64, f64); 5] = [(30.0, 20.0), (30.0, 2.0), (5.0, 2.0), (0.7, 2.0), (-0.6, 2.0)];
        let mut nonzero_points = 0usize;
        for &(lam, floor) in &points {
            let h_floor = 1e-6 * floor.abs().max(1.0);
            let h_lam = 1e-6 * lam.abs().max(1.0);
            let d_floor = |f: fn(f64, f64) -> f64| {
                (f(lam, floor + h_floor) - f(lam, floor - h_floor)) / (2.0 * h_floor)
            };
            let d_lam = |f: fn(f64, f64) -> f64| {
                (f(lam + h_lam, floor) - f(lam - h_lam, floor)) / (2.0 * h_lam)
            };
            let checks = [
                (
                    "antiderivative floor fourth",
                    jeffreys_antiderivative_floor_fourth_sensitivity(lam, floor),
                    d_floor(jeffreys_antiderivative_floor_third_sensitivity),
                ),
                (
                    "inverse floor third",
                    floored_inverse_floor_third_sensitivity(lam, floor),
                    d_floor(floored_inverse_floor_second_sensitivity),
                ),
                (
                    "inverse lambda floor second",
                    floored_inverse_lambda_floor_second_sensitivity(lam, floor),
                    d_lam(floored_inverse_floor_second_sensitivity),
                ),
                (
                    "inverse lambda lambda floor",
                    floored_inverse_lambda_lambda_floor_sensitivity(lam, floor),
                    d_lam(floored_inverse_lambda_floor_sensitivity),
                ),
            ];
            if checks.iter().any(|(_, analytic, _)| *analytic != 0.0) {
                nonzero_points += 1;
            }
            for (label, analytic, fd) in checks {
                assert!(
                    (fd - analytic).abs() <= 1e-6 * analytic.abs().max(1.0),
                    "{label} desync at (λ={lam}, floor={floor}): fd={fd:e} analytic={analytic:e}"
                );
            }
        }
    ...
    #[test]
    pub(crate) fn simple_eigenvector_second_drift_matches_fd_of_the_first_2894() {
        // `A(t, s) = A0 + t·X + s·Y + t·s·Z`: along s the first eigenvector derivative in t reads
        // the moving direction `X + s·Z`, so its central difference in s is the mixed second
        // derivative with `E_uw = Z`. Compared in ambient coordinates, with the eigenvector's
        // sign aligned to the base.
        fn argmin(values: &Array1<f64>) -> usize {
            (0..values.len())
                .min_by(|&i, &j| values[i].total_cmp(&values[j]))
                .expect("non-empty spectrum")
        ...
        let x = symmetric(array![
            [0.4, -0.2, 0.1, 0.3],
            [0.1, 0.25, -0.3, 0.05],
            [0.2, -0.1, -0.35, 0.15],
            [-0.05, 0.2, 0.1, 0.5]
        ]);
        let y = symmetric(array![
            [-0.3, 0.15, 0.2, -0.1],
            [0.25, 0.4, 0.05, 0.2],
            [-0.15, 0.3, 0.1, -0.25],
            [0.1, -0.05, 0.35, -0.2]
        ]);
        let zmat = symmetric(array![
            [0.2, 0.35, -0.25, 0.1],
            [-0.1, -0.15, 0.3, 0.2],
            [0.3, 0.05, 0.45, -0.1],
            [0.15, 0.25, -0.2, 0.3]
        ]);
        let (base_evals, base_evecs) = a0.eigh(Side::Lower).expect("symmetric spectrum");
        let tie_of = |values: &Array1<f64>| {
            64.0 * f64::EPSILON * values.iter().fold(1.0_f64, |acc, value| acc.max(value.abs()))
        };
        for (label, pick) in [("min", argmin as fn(&Array1<f64>) -> usize), ("max", argmax)] {
            let e = pick(&base_evals);
            let rotate = |matrix: &Array2<f64>, basis: &Array2<f64>| basis.t().dot(matrix).dot(basis);
            let reduced = simple_eigenvector_second_drift(
                &base_evals,
                tie_of(&base_evals),
                e,
                &rotate(&x, &base_evecs),
                &rotate(&y, &base_evecs),
                &rotate(&zmat, &base_evecs),
            );
            let analytic = base_evecs.dot(&reduced);
            let first_at = |s: f64| {
                let (evals, mut evecs) =
                    (&a0 + &(&y * s)).eigh(Side::Lower).expect("perturbed spectrum");
                let e_s = pick(&evals);
                if evecs.column(e_s).dot(&base_evecs.column(e)) < 0.0 {
                    evecs.column_mut(e_s).mapv_inplace(|value| -value);
                }
                let direction = rotate(&(&x + &(&zmat * s)), &evecs);
                let tie = tie_of(&evals);
                let reduced_first = Array1::from_shape_fn(evals.len(), |k| {
                    direction[[k, e_s]] * simple_eigenvalue_gap_inverse(&evals, tie, e_s, k)
                });
                evecs.dot(&reduced_first)
            };
            let step = 1.0e-5;
            let fd = (first_at(step) - first_at(-step)) / (2.0 * step);
            let scale = fd.iter().fold(0.0_f64, |acc, value| acc.max(value.abs()));
            assert!(scale > 1.0e-4, "{label}: the fixture must exercise a nonzero second drift");
            let error = (&analytic - &fd).iter().fold(0.0_f64, |acc, value| acc.max(value.abs()));
            assert!(
                error <= 1.0e-6 * scale.max(1.0),
                "{label}: analytic={analytic:?} fd={fd:?} error={error:e}"
            );
        }
    }
