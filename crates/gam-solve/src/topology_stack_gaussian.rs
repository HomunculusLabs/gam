...
mod tests {
        ...
        for (i, row) in expected.iter().enumerate() {
            ...
            for (k, &exp) in row.iter().enumerate() {
                ...
                if exp == f64::NEG_INFINITY {
                    assert!(g == f64::NEG_INFINITY, "[{i},{k}] expected -inf, got {g}");
                } else {
    ...
    fn parity_fx2_6x4_level_090_with_degenerate_and_nonfinite() {
        ...
        let means: Vec<Vec<f64>> = vec![
            vec![
                -0.507036, 2.843153, -2.377604, -0.33178, -1.651155, -0.902601,
            ],
            vec![
                -1.977385, -0.396524, -0.551639, 1.756306, 0.375639, -1.821592,
            ],
        ...
        let lowers: Vec<Vec<f64>> = vec![
            vec![
                -2.504768, 2.050415, -3.671604, -1.28709, -3.165972, -1.907575,
            ],
            vec![
                -3.390306, -1.1336, -1.673042, 1.160565, -0.563719, -3.436596,
            ],
            vec![
                -1.218091, -0.085092, 1.161906, -1.365173, 0.009935, -1.066681,
            ],
            vec![
                -3.139857, -2.839459, -3.97128, -1.390677, -2.284281, -0.526211,
            ],
        ];
        let uppers: Vec<Vec<f64>> = vec![
