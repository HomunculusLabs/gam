...
pub(crate) fn bessel_k_temme(mu: f64, x: f64) -> (f64, f64) {
    ...
    loop {
        ...
        if !(next.is_finite() && next1.is_finite()) {
            // SAFETY: `bessel_k` admits only finite ν and finite 0 < x, so a non-finite
            // partial sum is a broken invariant, and no finite value may stand in for it.
...
pub(crate) fn bessel_k_steed_cf2(mu: f64, x: f64) -> (f64, f64) {
    ...
    loop {
        ...
        if !(next_h.is_finite() && next_s.is_finite()) {
            // SAFETY: `bessel_k` admits only finite ν and finite x > 0, so a non-finite
            // iterate is a broken invariant, and no finite value may stand in for it.
