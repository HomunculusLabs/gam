...
impl RadialScalarKind {
    ...
    /// argument t (see `crate::basis::input_loc_derivatives`).
    ///
...
///
/// `StreamingMaternEvaluator` and `StreamingBSplineEvaluator` differ only in
/// how a single row chunk of the design is materialized (`for_row_chunk`) and
/// the chunk size policy (`chunk_rows`); every other operator method — the
/// chunked matvec, transpose-matvec, weighted Gram and dense materialization —
/// is identical boilerplate over that one primitive. This trait carries those
/// shared methods as defaults keyed on `for_row_chunk`, so each evaluator
/// implements only the per-basis pieces. The `NAME` const bakes the struct
/// name into the panic/error strings so diagnostics stay per-evaluator.
trait ChunkedDesign {
