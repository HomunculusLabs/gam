...
fn duchon_resolve_chart_reproduces_the_cold_build() {
    for (label, (data, spec)) in [
        ...
        assert_eq!(
            resolved.spec.radial_reparam.as_ref(),
            cold_v.as_ref(),
            "[{label}] resolved data-metric reparam V differs from the cold build's, in presence or value"
        );
        ...
        assert_eq!(
            resolved.identifiability_transform.as_ref(),
            cold_t.as_ref(),
            "[{label}] resolved identifiability transform differs from the cold build's, in presence \
             or value — it must be read off the V-rotated design"
        );
