...
| `enable_outer_gradient_fd_capture` | **Retired with the production finite difference it armed (#2901 V3).** The outer runner differences nothing. A test registers `gam_solve::estimate::outer_eval_capture::observe_next_outer_seed`, receives an analytic `OuterSeedProbe` at the seed, and forms its own difference, as `crates/gam-models/tests/survival_acceptance/follow_up_mode_response_fd_2765.rs` does. |
...
| `LocalAtlas::co_collapse_candidates` | **Restored, then deleted by `e6fd4251e`** together with `CoCollapseCandidate` and its three pins, because nothing in production queried it. The exported `CoCollapseCandidate` report had survived the sweep with no producer, so the query came back as a pure function of the fitted atlas: well-conditioned transitions, mutual coverage `|shared| / min(patch sizes)`, and the Procrustes residual. The three historical #2280 pins (`co_collapse_flags_duplicate_charts_2280`, `co_collapse_thresholds_bracket_the_gate_2280`, `co_collapse_spares_healthy_swiss_roll_atlas_2280`) were restored beside it and deleted with it. |
...
| Retired: no product uses it | 246 |
| Narrowed to crate visibility | 8 |
...
records. A row narrowed under that rule reads `narrowed-to-crate`; its declaration
...
were reworded, and a third once the producer-less `RiemannianLBFGS` carrier that
outlived its `minimize` was deleted. Among the rows once marked defined elsewhere, `row_loss` is retired
