...
// The entire harness — including its `gam::solver::gpu_kernels` imports — only
// exists on Linux (the CUDA stack is linux-gated), so the compile-time gate
// implements the skip notice the doc above promises for other hosts.
#[cfg(target_os = "linux")]
mod harness {
...
fn main() -> std::process::ExitCode {
    println!("arrow_schur_hill_climb: linux-only CUDA harness; skipping.");
