...
cd ~/lane-2502
if ! cargo check -p gam-sae -p gam-pyffi 2> ~/i2502/check2.log; then
  log "CARGO CHECK FAILED (see check2.log)"
  exit 1
fi
log "check green; building wheels4"
if ! ~/venv2283/bin/maturin build --release --out ~/i2502/wheels4 -i ~/venv2283/bin/python > ~/i2502/build4.log 2>&1; then
  log "WHEEL BUILD FAILED (see build4.log)"
  exit 1
fi
W=$(ls -t ~/i2502/wheels4/*.whl | head -1)
...
log "wheels4 installed; relaunching fits"
cd ~/i2502
