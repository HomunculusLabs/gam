...
MINCO=${MINCO:-300}
PARSE=${PARSE:-2000}
# 0 = let the census derive its own budget: B + 1 = m/alpha is the smallest
# budget at which every e-BH rank is reachable and the largest that buys
# anything (more draws make `no surrogate reached the observation` harder for a
# real signal, for no gain). Set a positive value only to compare statistics at
# a fixed budget.
DRAWS=${DRAWS:-0}
ALPHA=${ALPHA:-0.05}
OUT=$HOME/census_out
