...
class _GaussianRemlFitWithConstraintsFn(torch.autograd.Function):
    ...
    * **Active cert (non-empty active set):** the math identity is
    ...
    def forward(
        ...
        # The Rust forward returns the honest KKT active set: the rows binding
        # at the accepted β under the solver's own primal-feasibility band
        # (|a_i·β − b_i| within ACTIVE_SET_PRIMAL_FEASIBILITY_TOL, row/solution
        # scaled). The analytic VJP branches on this set (empty ⇒
        # envelope/interior-cert path; non-empty ⇒ deferred tangent-projection
        # path), so it is consumed directly — no Python recompute.
