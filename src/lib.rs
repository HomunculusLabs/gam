...
/// macro is used only by in-crate `#[cfg(test)]` unit tests, which call it
/// through its defining-crate path
/// `gam_linalg_test_support::assert_central_difference_array!` (it lives in that
/// dev-only leaf so crates needing it no longer pull the model layer into their
/// test builds), so it is intentionally NOT re-exported onto the public surface.
