...
/// Every way `channel` breaks the contract, one line each.
fn channel_violations(row: &str, channel: &GradientChannel, rel_tol: f64) -> Vec<String> {
    if channel.analytic.len() != channel.fd.len() {
        return vec![format!(
            "{row}/{} length mismatch: analytic={} fd={}",
            channel.name,
    ...
    for idx in 0..channel.analytic.len() {
        ...
        if !(analytic.is_finite() && fd.is_finite()) {
...
fn gradient_is_differential_contract_gate() {
    ...
    // Every row is evaluated before the gate asserts, so one red row cannot hide
    // whether the others hold.
    let mut violations = Vec::new();
    for row in rows {
        ...
        for channel in channels {
            violations.extend(channel_violations(row.name, &channel, row.rel_tol));
        }
    }
    assert!(
        violations.is_empty(),
        "{} gradient contract violation(s):\n{}",
        violations.len(),
        violations.join("\n")
    );
}
...
/// Central difference of the SAE penalized objective along one input of `term`, taken on
/// `term` itself and restored afterwards. `assemble_arrow_schur` freezes the decoder-repulsion,
/// barrier-coactivation and amplitude-barrier gates on the term it assembles, and the gradient
/// it returns differentiates the objective under those frozen gates. `SaeManifoldTerm::clone`
/// drops all three gates, so a clone prices the separation barrier from live routing: a
/// different objective, carrying a logit derivative the frozen-gate gradient deliberately
/// omits (#1625, c3183566a).
fn sae_central_difference(
    term: &mut SaeManifoldTerm,
    z: &Array2<f64>,
    rho: &SaeManifoldRho,
    base: f64,
    h: f64,
    mut set: impl FnMut(&mut SaeManifoldTerm, f64),
) -> f64 {
    set(term, base + h);
    let plus = sae_value(term, z, rho);
    set(term, base - h);
    let minus = sae_value(term, z, rho);
    set(term, base);
    (plus - minus) / (2.0 * h)
}
...
fn sae_euclidean_line_row() -> Vec<GradientChannel> {
    ...
    for row in coord_probes {
        ...
        coord_fd.push(sae_central_difference(
            &mut term,
            &z,
            &rho,
            base,
            FD_STEP,
            |t, v| set_coord(t, 0, row, 0, v),
        ));
    }
    ...
    for (basis_col, out_col) in decoder_probes {
        ...
        decoder_fd.push(sae_central_difference(
            &mut term,
            &z,
            &rho,
            base,
            FD_STEP,
            |t, v| t.atoms[0].decoder_coefficients_mut()[[basis_col, out_col]] = v,
        ));
    }
...
fn sae_k2_periodic_overlap_row() -> Vec<GradientChannel> {
    ...
    for row in [0usize, 11, 28] {
        ...
        let central = |term: &mut SaeManifoldTerm, h: f64| {
            let base = term.assignment.logits[[row, 0]];
            sae_central_difference(term, &z, &rho, base, h, |t, v| {
                t.assignment.logits[[row, 0]] = v;
            })
        };
        let fd = central(&mut term, FD_STEP);
        logit_fd.push(fd);
        ...
        let rel = (analytic - fd).abs() / analytic.abs().max(fd.abs()).max(REL_FLOOR);
        if rel >= REL_TOL {
            let coarse = central(&mut term, FD_STEP * 10.0);
            let fine = central(&mut term, FD_STEP / 10.0);
    ...
    for (atom, basis_col, out_col) in [(0usize, 1usize, 1usize), (1, 2, 2), (1, 0, 0)] {
        ...
        decoder_fd.push(sae_central_difference(
            ...
            base,
            FD_STEP,
            |t, v| t.atoms[atom].decoder_coefficients_mut()[[basis_col, out_col]] = v,
        ));
    }
