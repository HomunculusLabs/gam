...
//! per-atom **lens** ([`gam::terms::sae::inference::atom_lens::atom_two_lens`]).
//!
...
use gam::terms::sae::inference::atom_lens::{AtomTwoLensReport, atom_two_lens};
...
fn entry<'a>(
    ...
    name: &str,
) -> &'a gam::terms::sae::inference::atom_lens::AtomLensEntry {
    report
        .atoms
        .iter()
        .find(|e| e.name == name)
...
fn lens_flags_loud_represented_not_used_and_quiet_used() {
    ...
    let euclid_report = atom_two_lens(&term, &euclid, None).expect("Euclidean lens report");
    assert_eq!(
    ...
    let report = atom_two_lens(&term, &fisher, None).expect("Fisher lens report");
    assert!(matches!(
