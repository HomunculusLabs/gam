...
// under the 360s suite cap on top of the ~84s GAM REML fit. gam runs
// `NUTS_CHAINS` chains and ends its warmup when adaptation has stabilized; the
// PyMC baseline runs the same number of chains, with its tune matched to draws
// so its step-size / mass-matrix adaptation is fully converged.
const REAL_DATA_POSTERIOR_SAMPLES: usize = 250;
const REAL_DATA_POSTERIOR_WARMUP: usize = 250;
const REAL_DATA_POSTERIOR_CHAINS: usize = gam::hmc::NUTS_CHAINS;
...
fn gam_nuts_binomial_logit_recovers_truth_and_is_calibrated() {
    ...
    // on a ~p-dimensional posterior. gam runs `NUTS_CHAINS` chains, so R-hat is
    // meaningful, and its warmup ends when its adaptation has stabilized.
