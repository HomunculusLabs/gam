...
#[test]
fn smooth_quadratic_recovery() {
    init_parallelism();
    let d = mk_1d(200, |t| t * t, (0.0, 1.0), 0.05, 7);
    let p = fit_1d("y~s(x,k=10)", d, &[0.0, 0.5, 1.0]);
    assert!(p.iter().all(|v| v.is_finite()));
}
#[test]
fn smooth_cubic_recovery() {
    init_parallelism();
...
#[test]
fn smooth_sin_recovery() {
    init_parallelism();
    let d = mk_1d(200, |t| (PI * t).sin(), (0.0, 1.0), 0.05, 7);
    let p = fit_1d("y~s(x,k=15)", d, &[0.1, 0.5, 0.9]);
    assert!(p.iter().all(|v| v.is_finite()));
    assert!(p[1] > 0.5);
}
#[test]
fn smooth_exponential() {
    init_parallelism();
    let d = mk_1d(200, |t| t.exp(), (0.0, 1.0), 0.05, 7);
    let p = fit_1d("y~s(x,k=15)", d, &[0.1, 0.9]);
    assert!(p[1] > p[0]);
}
#[test]
fn smooth_log_recovers_monotone() {
    init_parallelism();
    let d = mk_1d(200, |t| (t + 0.1).ln(), (0.0, 1.0), 0.05, 7);
    let p = fit_1d("y~s(x,k=15)", d, &[0.1, 0.5, 0.9]);
    assert!(p[0] < p[1] && p[1] < p[2]);
}
#[test]
fn smooth_with_offset_in_y() {
    init_parallelism();
    let d = mk_1d(200, |t| 100.0 + t.cos(), (0.0, TAU), 0.05, 7);
    let p = fit_1d("y~s(x,k=15)", d, &[0.0, PI]);
    assert!(p[0] > 95.0 && p[1] < 101.0);
}
#[test]
fn smooth_negative_y_range() {
    init_parallelism();
    let d = mk_1d(200, |t| -5.0 + (PI * t).sin(), (0.0, 1.0), 0.05, 7);
    let p = fit_1d("y~s(x,k=15)", d, &[0.5]);
    assert!(p[0] < -3.5 && p[0] > -5.5);
}
#[test]
...
#[test]
fn periodic_with_high_k() {
    init_parallelism();
    let d = mk_1d(200, |t| t.cos(), (0.0, TAU), 0.05, 7);
    let p = fit_1d(
        "y~s(x,periodic=true,period=6.283185307179586,k=40)",
        d,
        &[0.0, PI],
    );
    assert!((p[0] - 1.0).abs() < 0.3 && (p[1] - (-1.0)).abs() < 0.3);
}
#[test]
fn periodic_recovers_cos2t() {
    init_parallelism();
    let d = mk_1d(200, |t| (2.0 * t).cos(), (0.0, TAU), 0.05, 7);
    let p = fit_1d(
        "y~s(x,periodic=true,period=6.283185307179586,k=20)",
        d,
        &[0.0, PI / 2.0],
    );
    assert!((p[0] - 1.0).abs() < 0.3 && (p[1] + 1.0).abs() < 0.3);
...
#[test]
fn bc_anchored_with_few_data() {
    init_parallelism();
    let d = mk_1d(50, |t| (PI * t).sin(), (0.0, 1.0), 0.05, 7);
    let p = fit_1d("y~s(x,bc=anchored,k=10)", d, &[0.5]);
    assert!(p[0].is_finite());
}
#[test]
fn bc_clamped_with_few_data() {
    init_parallelism();
    let d = mk_1d(50, |t| (PI * t).sin(), (0.0, 1.0), 0.05, 7);
    let p = fit_1d("y~s(x,bc=clamped,k=10)", d, &[0.5]);
    assert!(p[0].is_finite());
}
#[test]
fn sphere_dense_polar() {
    init_parallelism();
    let d = mk_sphere(400, |lat, _| 0.5 + 0.3 * lat.to_radians().sin(), 0.05, 7);
    let p = fit_sphere("y~sphere(lat,lon,k=30)", d, &[(85.0, 0.0), (-85.0, 0.0)]);
    assert!(p.iter().all(|v| v.is_finite()));
}
#[test]
...
#[test]
fn smooth_fits_logistic_dose_response() {
    init_parallelism();
    let d = mk_1d(
        200,
        |t| 1.0 / (1.0 + (-(10.0 * t - 5.0)).exp()),
        (0.0, 1.0),
        0.05,
        7,
    );
    let p = fit_1d("y~s(x,k=15)", d, &[0.1, 0.5, 0.9]);
    assert!(p[0] < 0.2 && p[1] > 0.3 && p[2] > 0.8);
}
#[test]
fn periodic_with_large_period() {
    init_parallelism();
    let d = mk_1d(200, |t| (TAU * t / 100.0).cos(), (0.0, 100.0), 0.05, 7);
    let p = fit_1d(
        "y~s(x,periodic=true,period=100.0,k=15)",
        d,
        &[0.0, 50.0, 100.0],
    );
    assert!((p[0] - p[2]).abs() < 0.1);
}
#[test]
fn smooth_step_then_constant_recovers_inflection() {
    init_parallelism();
    let d = mk_1d(
        200,
        |t| {
    ...
    let p = fit_1d("y~s(x,k=20)", d, &[0.2, 0.5, 0.8]);
    assert!(p[0] < 0.4 && p[2] > 0.6);
}
#[test]
fn smooth_with_periodic_aliased_data() {
    ...
    let y: Vec<f64> = t
        .iter()
        .map(|&x| (2.0 * x).cos() + 0.3 * (5.0 * x).sin() + noise.sample(&mut rng))
        .collect();
...
#[test]
fn sphere_fits_uniform_constant_truth() {
    init_parallelism();
    let d = mk_sphere(300, |_, _| 2.5, 0.1, 7);
    let p = fit_sphere(
        "y~sphere(lat,lon,k=20)",
        d,
        &[(0.0, 0.0), (45.0, 90.0), (-45.0, -90.0)],
    );
    for v in &p {
...
#[test]
fn sphere_fits_lat_dependent_truth() {
    init_parallelism();
    let d = mk_sphere(300, |lat, _| 0.5 * lat.to_radians().sin(), 0.05, 7);
    let p = fit_sphere("y~sphere(lat,lon,k=20)", d, &[(60.0, 0.0), (-60.0, 0.0)]);
    assert!(p[0] > p[1]);
}
#[test]
fn smooth_with_no_intercept_explicit() {
    init_parallelism();
    let d = mk_1d(200, |t| (PI * t).sin(), (0.0, 1.0), 0.05, 7);
    let p = fit_1d("y~s(x,k=10)", d, &[0.5]);
    assert!((p[0] - 1.0).abs() < 0.2);
}
#[test]
fn smooth_with_intercept_match() {
    init_parallelism();
    let d = mk_1d(200, |t| 3.0 + 0.5 * t, (0.0, 1.0), 0.05, 7);
    let p = fit_1d("y~s(x,k=10)", d, &[0.0, 1.0]);
    assert!((p[0] - 3.0).abs() < 0.3 && (p[1] - 3.5).abs() < 0.3);
}
