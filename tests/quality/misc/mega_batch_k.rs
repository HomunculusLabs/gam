...
#[test]
fn tensor_2d_recovers_x() {
    let d = mk_2d(300, |a, _| a, (0.0, 1.0), (0.0, 1.0), 0.05, 7);
    let p = fit2d("y~te(a,b,k=5)", d, &[(0.2, 0.5), (0.8, 0.5)]);
    assert!(p[0] < p[1]);
}
#[test]
fn tensor_2d_recovers_y() {
    let d = mk_2d(300, |_, b| b, (0.0, 1.0), (0.0, 1.0), 0.05, 7);
    let p = fit2d("y~te(a,b,k=5)", d, &[(0.5, 0.2), (0.5, 0.8)]);
    assert!(p[0] < p[1]);
}
#[test]
fn tensor_2d_interaction() {
    let d = mk_2d(300, |a, b| a * b, (0.0, 1.0), (0.0, 1.0), 0.05, 7);
    let p = fit2d("y~te(a,b,k=5)", d, &[(0.9, 0.9), (0.1, 0.1)]);
    assert!(p[0] > p[1]);
}
#[test]
fn tensor_2d_finite_at_origin() {
    let d = mk_2d(200, |a, b| a + b, (0.0, 1.0), (0.0, 1.0), 0.05, 7);
    let p = fit2d("y~te(a,b,k=5)", d, &[(0.0, 0.0)]);
...
#[test]
fn periodic_with_high_freq_truth() {
    let d = mk_1d(300, |t| (8.0 * t).cos(), (0.0, TAU), 0.05, 7);
    let p = fit1d(
        "y~s(x,periodic=true,period=6.283185307179586,k=25)",
        d,
        &[0.0],
    );
    assert!(p[0].is_finite());
...
#[test]
fn periodic_with_low_freq_truth() {
    let d = mk_1d(300, |t| (0.5 * t).cos(), (0.0, TAU), 0.05, 7);
    let p = fit1d(
        "y~s(x,periodic=true,period=6.283185307179586,k=15)",
        d,
        &[0.0],
    );
    assert!(p[0].is_finite());
...
#[test]
fn smooth_with_normal_noise_high_sigma() {
    let d = mk_1d(200, |t| t.sin(), (0.0, TAU), 1.0, 7);
    let p = fit1d("y~s(x,k=10)", d, &[PI]);
    assert!(p[0].is_finite());
}
#[test]
fn smooth_recovers_quadratic_in_offset_domain() {
    let d = mk_1d(200, |t| (t - 5.0) * (t - 5.0), (0.0, 10.0), 0.05, 7);
    let p = fit1d("y~s(x,k=15)", d, &[2.0, 5.0, 8.0]);
    assert!(p[1] < p[0] && p[1] < p[2]);
}
#[test]
fn bc_clamped_handles_constant_truth() {
    let d = mk_1d(200, |_| 3.0, (0.0, 1.0), 0.1, 7);
    let p = fit1d("y~s(x,bc=clamped,k=15)", d, &[0.5]);
    assert!((p[0] - 3.0).abs() < 0.2);
}
#[test]
fn bc_anchored_handles_polynomial_truth() {
    let d = mk_1d(200, |t| t * t, (0.0, 1.0), 0.05, 7);
    let p = fit1d("y~s(x,bc=anchored,k=15)", d, &[0.5]);
    assert!(p[0].is_finite());
}
#[test]
fn sphere_recovers_lat_squared() {
    let d = mk_2d(
        300,
        |lat, _| lat.to_radians().sin().powi(2),
        (-70.0, 70.0),
        (-179.0, 179.0),
        0.05,
        7,
    );
    let p = fit2d("y~sphere(a,b,k=20)", d, &[(0.0, 0.0), (60.0, 0.0)]);
    assert!(p[1] > p[0]);
}
#[test]
fn sphere_recovers_lon_dependent() {
    let d = mk_2d(
        300,
        |_, lon| lon.to_radians().cos(),
        (-70.0, 70.0),
        (-179.0, 179.0),
        0.05,
        7,
    );
    let p = fit2d("y~sphere(a,b,k=20)", d, &[(0.0, 0.0), (0.0, 90.0)]);
    assert!(p[0] > p[1]);
}
#[test]
...
#[test]
fn cylinder_recovers_h_only() {
    let d = mk_2d(300, |_, h| 0.5 + 0.4 * h, (0.0, TAU), (-1.0, 1.0), 0.05, 7);
    let p = fit2d(
        "y~te(a,b,bc=['periodic','natural'],period=[2*pi,None],k=5)",
        d,
        &[(0.0, -0.5), (0.0, 0.5)],
    );
    assert!(p[0] < p[1]);
...
#[test]
fn cylinder_recovers_theta_only() {
    let d = mk_2d(300, |th, _| th.cos(), (0.0, TAU), (-1.0, 1.0), 0.05, 7);
    let p = fit2d(
        "y~te(a,b,bc=['periodic','natural'],period=[2*pi,None],k=5)",
        d,
        &[(0.0, 0.0), (PI, 0.0)],
...
#[test]
fn cylinder_seam_continuity_finite() {
    let d = mk_2d(
        300,
        |th, h| th.cos() + 0.3 * h,
        (0.0, TAU),
...
#[test]
fn torus_recovers_double_periodic() {
    let d = mk_2d(
        300,
        |u, v| u.cos() + v.sin(),
        (0.0, TAU),
    ...
    let p = fit2d(
        "y~te(a,b,bc=['periodic','periodic'],period=[2*pi,2*pi],k=5)",
        d,
        &[(0.0, 0.0)],
    );
    assert!(p[0].is_finite());
...
#[test]
fn matern_nu_half_1d() {
    let d = mk_1d(200, |t| t.cos(), (0.0, TAU), 0.05, 7);
    let p = fit1d("y~matern(x,nu=1/2)", d, &[PI]);
    assert!(p[0].is_finite());
}
#[test]
fn matern_nu_three_halves_1d() {
    let d = mk_1d(200, |t| t.cos(), (0.0, TAU), 0.05, 7);
    let p = fit1d("y~matern(x,nu=3/2)", d, &[PI]);
    assert!(p[0].is_finite());
}
#[test]
fn matern_nu_seven_halves_1d() {
    let d = mk_1d(200, |t| t.cos(), (0.0, TAU), 0.05, 7);
    let p = fit1d("y~matern(x,nu=7/2)", d, &[PI]);
    assert!(p[0].is_finite());
}
#[test]
fn matern_recovers_polynomial() {
    let d = mk_1d(200, |t| t * t * t, (0.0, 1.0), 0.05, 7);
    let p = fit1d("y~matern(x,nu=5/2)", d, &[0.5]);
    assert!((p[0] - 0.125).abs() < 0.1);
}
#[test]
fn matern_with_explicit_centers() {
    let d = mk_1d(200, |t| t.sin(), (0.0, 1.0), 0.05, 7);
    let p = fit1d("y~matern(x,nu=5/2,centers=20)", d, &[0.5]);
    assert!(p[0].is_finite());
}
#[test]
fn periodic_handles_quarter_period_data() {
    let d = mk_1d(200, |t| t.cos(), (0.0, PI / 2.0), 0.05, 7);
    let p = fit1d(
        "y~s(x,periodic=true,period=6.283185307179586,k=10)",
        d,
        &[0.0, PI],
    );
    assert!(p.iter().all(|v| v.is_finite()));
...
#[test]
fn periodic_handles_three_quarter_period_data() {
    let d = mk_1d(200, |t| t.cos(), (0.0, 3.0 * PI / 2.0), 0.05, 7);
    let p = fit1d(
        "y~s(x,periodic=true,period=6.283185307179586,k=10)",
        d,
        &[0.0],
...
#[test]
fn smooth_with_explicit_intercept_excluded() {
    let d = mk_1d(200, |t| t.sin() + 1.0, (0.0, TAU), 0.05, 7);
    let p = fit1d("y~s(x,k=10)", d, &[PI / 2.0]);
    assert!((p[0] - 2.0).abs() < 0.3);
}
#[test]
fn bc_anchored_with_large_data_range_works() {
    let d = mk_1d(200, |t| (0.5 * t).sin(), (0.0, 4.0), 0.05, 7);
    let p = fit1d("y~s(x,bc=anchored,k=15)", d, &[2.0]);
    assert!(p[0].is_finite());
}
#[test]
fn bc_clamped_with_truth_zero_at_endpoints() {
    let d = mk_1d(200, |t| (PI * t).sin(), (0.0, 1.0), 0.05, 7);
    let p = fit1d("y~s(x,bc=clamped,k=15)", d, &[0.5]);
    assert!((p[0] - 1.0).abs() < 0.3);
}
#[test]
...
#[test]
fn te_with_high_k() {
    let d = mk_2d(400, |a, b| a * a + b * b, (0.0, 1.0), (0.0, 1.0), 0.05, 7);
    let p = fit2d("y~te(a,b,k=10)", d, &[(0.5, 0.5)]);
    assert!(p[0].is_finite());
}
#[test]
fn te_with_low_k() {
    let d = mk_2d(300, |a, b| a + b, (0.0, 1.0), (0.0, 1.0), 0.05, 7);
    let p = fit2d("y~te(a,b,k=4)", d, &[(0.5, 0.5)]);
    assert!(p[0].is_finite());
}
#[test]
fn smooth_at_data_extremes() {
    let d = mk_1d(200, |t| t, (0.1, 0.9), 0.05, 7);
    let p = fit1d("y~s(x,k=10)", d, &[0.05, 0.95]);
    assert!(p[0] < p[1]);
}
#[test]
fn periodic_at_seam_doubled() {
    let d = mk_1d(200, |t| t.cos(), (0.0, TAU), 0.05, 7);
    let p = fit1d(
        "y~s(x,periodic=true,period=6.283185307179586)",
        d,
        &[0.0, TAU, 2.0 * TAU],
    );
    assert!((p[0] - p[1]).abs() < 1e-9 && (p[0] - p[2]).abs() < 1e-9);
}
#[test]
fn sphere_grid_data_works() {
    ...
    for i in 0..10 {
        for j in 0..10 {
            ...
            let y = 0.5 + 0.3 * lat.to_radians().sin();
            rows.push(StringRecord::from(vec![
                lat.to_string(),
                lon.to_string(),
                y.to_string(),
...
#[test]
fn sphere_strict_lat_bound_at_90() {
    init_parallelism();
    let mut rows = Vec::new();
    let h = ["lat", "lon", "y"].into_iter().map(String::from).collect();
    for i in 0..30 {
        let lat = -80.0 + i as f64 * 5.0;
        rows.push(StringRecord::from(vec![
            lat.to_string(),
            "0.0".to_string(),
            "1.0".to_string(),
...
fn smooth_x_squared_inflection_at_zero() {
    let d = mk_1d(200, |t| t * t, (-1.0, 1.0), 0.05, 7);
    let p = fit1d("y~s(x,k=15)", d, &[-1.0, 0.0, 1.0]);
    assert!(p[0] > p[1] && p[2] > p[1]);
}
#[test]
fn periodic_double_amplitude() {
    let d = mk_1d(200, |t| 2.0 * t.cos(), (0.0, TAU), 0.05, 7);
    let p = fit1d(
        "y~s(x,periodic=true,period=6.283185307179586,k=15)",
        d,
        &[0.0, PI],
...
#[test]
fn smooth_finite_at_data_centroid() {
    let d = mk_1d(300, |t| t.exp(), (0.0, 1.0), 0.05, 7);
    let p = fit1d("y~s(x,k=10)", d, &[0.5]);
    assert!(p[0] > 1.0 && p[0] < 2.0);
}
#[test]
fn bc_anchored_pred_at_data_min_endpoint() {
    let d = mk_1d(200, |t| t, (0.1, 0.9), 0.05, 7);
    let p = fit1d("y~s(x,bc=anchored,k=10)", d, &[0.1, 0.9]);
    assert!((p[0] - p[1]).abs() < 0.1);
}
#[test]
fn bc_clamped_pred_at_data_min_endpoint() {
    let d = mk_1d(200, |t| t, (0.1, 0.9), 0.05, 7);
    let p = fit1d("y~s(x,bc=clamped,k=10)", d, &[0.1, 0.9]);
    assert!(p.iter().all(|v| v.is_finite()));
}
#[test]
fn smooth_recovers_zero_truth() {
    let d = mk_1d(200, |_| 0.0, (0.0, 1.0), 0.05, 7);
    let p = fit1d("y~s(x,k=10)", d, &[0.5]);
    assert!(p[0].abs() < 0.1);
}
#[test]
fn periodic_recovers_zero_truth() {
    let d = mk_1d(200, |_| 0.0, (0.0, TAU), 0.05, 7);
    let p = fit1d("y~s(x,periodic=true,period=6.283185307179586)", d, &[PI]);
    assert!(p[0].abs() < 0.1);
}
#[test]
fn sphere_recovers_zero_truth() {
    let d = mk_2d(300, |_, _| 0.0, (-70.0, 70.0), (-179.0, 179.0), 0.05, 7);
    let p = fit2d("y~sphere(a,b,k=15)", d, &[(45.0, 0.0)]);
    assert!(p[0].abs() < 0.1);
}
#[test]
fn te_recovers_zero_truth() {
    let d = mk_2d(300, |_, _| 0.0, (0.0, 1.0), (0.0, 1.0), 0.05, 7);
    let p = fit2d("y~te(a,b,k=5)", d, &[(0.5, 0.5)]);
    assert!(p[0].abs() < 0.1);
}
#[test]
...
fn smooth_recovers_decreasing_truth() {
    let d = mk_1d(200, |t| 1.0 - t, (0.0, 1.0), 0.05, 7);
    let p = fit1d("y~s(x,k=10)", d, &[0.1, 0.9]);
    assert!(p[0] > p[1]);
}
#[test]
fn smooth_recovers_increasing_truth() {
    let d = mk_1d(200, |t| t * 3.0, (0.0, 1.0), 0.05, 7);
    let p = fit1d("y~s(x,k=10)", d, &[0.1, 0.9]);
    assert!(p[0] < p[1]);
}
#[test]
fn smooth_recovers_v_shape() {
    let d = mk_1d(200, |t| (t - 0.5).abs(), (0.0, 1.0), 0.05, 7);
    let p = fit1d("y~s(x,k=15)", d, &[0.1, 0.5, 0.9]);
    assert!(p[1] < p[0] && p[1] < p[2]);
}
#[test]
fn smooth_recovers_inverted_v_shape() {
    let d = mk_1d(200, |t| 0.5 - (t - 0.5).abs(), (0.0, 1.0), 0.05, 7);
    let p = fit1d("y~s(x,k=15)", d, &[0.1, 0.5, 0.9]);
    assert!(p[1] > p[0] && p[1] > p[2]);
}
#[test]
fn smooth_k_consistency_check() {
    let d = mk_1d(200, |t| t.sin(), (0.0, 1.0), 0.05, 7);
    let p10 = fit1d("y~s(x,k=10)", d.clone(), &[0.5]);
    let p20 = fit1d("y~s(x,k=20)", d, &[0.5]);
    assert!((p10[0] - p20[0]).abs() < 0.2);
}
#[test]
...
#[test]
fn te_with_disparate_scales() {
    let d = mk_2d(300, |a, b| a + b, (0.0, 1.0), (0.0, 1000.0), 0.05, 7);
    let p = fit2d("y~te(a,b,k=5)", d, &[(0.5, 500.0)]);
    assert!(p[0].is_finite());
}
#[test]
fn smooth_with_data_in_negative_range() {
    let d = mk_1d(200, |t| t.sin(), (-5.0, 5.0), 0.05, 7);
    let p = fit1d("y~s(x,k=15)", d, &[-2.0, 2.0]);
    assert!(p.iter().all(|v| v.is_finite()));
}
#[test]
fn bc_anchored_with_lots_of_data() {
    let d = mk_1d(1000, |t| (PI * t).sin(), (0.0, 1.0), 0.05, 7);
    let p = fit1d("y~s(x,bc=anchored,k=25)", d, &[0.5]);
    assert!((p[0] - 1.0).abs() < 0.2);
}
#[test]
fn sphere_with_lots_of_data() {
    let d = mk_2d(
        1000,
        |lat, _| 0.5 + 0.3 * lat.to_radians().sin(),
        (-70.0, 70.0),
        (-179.0, 179.0),
        0.05,
        7,
    );
    let p = fit2d("y~sphere(a,b,k=20)", d, &[(0.0, 0.0)]);
    assert!(p[0].is_finite());
}
#[test]
fn periodic_with_lots_of_data() {
    let d = mk_1d(1000, |t| t.cos(), (0.0, TAU), 0.05, 7);
    let p = fit1d(
        "y~s(x,periodic=true,period=6.283185307179586,k=30)",
        d,
        &[0.0],
    );
    assert!((p[0] - 1.0).abs() < 0.2);
...
#[test]
fn smooth_consistent_across_seeds() {
    let p7 = fit1d(
        "y~s(x,k=15)",
        mk_1d(200, |t| t.sin(), (0.0, 1.0), 0.05, 7),
        &[0.5],
    );
    let p11 = fit1d(
        "y~s(x,k=15)",
        mk_1d(200, |t| t.sin(), (0.0, 1.0), 0.05, 11),
        &[0.5],
    );
    assert!((p7[0] - p11[0]).abs() < 0.15);
}
#[test]
fn sphere_consistent_across_seeds() {
    ...
    let min = p_vals.iter().cloned().fold(f64::INFINITY, f64::min);
    assert!(max - min < 0.15);
}
#[test]
fn smooth_predictions_smooth_across_grid() {
    let d = mk_1d(200, |t| t.sin(), (0.0, 1.0), 0.05, 7);
    let xs: Vec<f64> = (0..50).map(|i| i as f64 / 49.0).collect();
    let p = fit1d("y~s(x,k=15)", d, &xs);
    for i in 1..p.len() {
        assert!((p[i] - p[i - 1]).abs() < 0.2);
    }
}
#[test]
fn periodic_predictions_smooth_across_grid() {
    let d = mk_1d(200, |t| t.cos(), (0.0, TAU), 0.05, 7);
    let xs: Vec<f64> = (0..50).map(|i| TAU * (i as f64) / 49.0).collect();
    let p = fit1d("y~s(x,periodic=true,period=6.283185307179586)", d, &xs);
    for i in 1..p.len() {
...
fn sphere_predictions_smooth_along_lat_band() {
    ...
    let pts: Vec<(f64, f64)> = (0..30)
        ...
        .collect();
    let p = fit2d("y~sphere(lat,lon,k=20)", d, &pts);
    for i in 1..p.len() {
        assert!((p[i] - p[i - 1]).abs() < 0.5);
    }
}
#[test]
fn smooth_with_large_data_count_finishes() {
    let d = mk_1d(5000, |t| t.sin(), (0.0, 1.0), 0.05, 7);
    let p = fit1d("y~s(x,k=15)", d, &[0.5]);
    assert!(p[0].is_finite());
}
#[test]
...
#[test]
fn te_with_minimum_basis_finite() {
    let d = mk_2d(200, |a, b| a + b, (0.0, 1.0), (0.0, 1.0), 0.05, 7);
    let p = fit2d("y~te(a,b,k=4)", d, &[(0.5, 0.5)]);
    assert!(p[0].is_finite());
}
#[test]
fn periodic_with_k_equal_basis_minimum() {
    let d = mk_1d(200, |t| t.cos(), (0.0, TAU), 0.05, 7);
    let p = fit1d(
        "y~s(x,periodic=true,period=6.283185307179586,k=4)",
        d,
        &[PI],
    );
    assert!(p[0].is_finite());
...
fn all_smooth_families_in_one_formula() {
    ...
    for _ in 0..400 {
        ...
        let th = ut.sample(&mut rng);
        let lat = ul.sample(&mut rng);
        let lon = un.sample(&mut rng);
        let y = 0.3 + 0.2 * x + 0.3 * th.cos() + 0.2 * lat.to_radians().sin() + no.sample(&mut rng);
        r.push(StringRecord::from(vec![
            x.to_string(),
            th.to_string(),
            lat.to_string(),
    ...
    let result = fit_from_formula(
        "y~s(x,k=6)+s(theta,periodic=true,period=6.283185307179586,k=6)+sphere(lat,lon,k=10)",
        &d,
        &cfg,
    )
    .expect("fit");
