...
// TEMPORARY gam#2695 probe — the SHIPPED witness
// (`survival_location_scale_saved_fit_preserves_linkwiggle_metadata`) run with a
// logging backend installed, so the `[P2695-PHI0]` instrument that `3c530dbda`
// landed can say whether the objective's Φ term falls back to `0.0` at trial
// points while the baseline carries a finite one. Asserts nothing; the witness
// itself is the assertion.
#[test]
fn probe_2695_witness_trail() {
    gam_runtime::test_support::install_diagnostic_logger();
    let dir = tempdir().unwrap_or_else(|e| panic!("{} failed: {:?}", "tempdir", e));
    let csv_path = dir.path().join("small_surv_linkwiggle.csv");
    let out_path = dir.path().join("surv_linkwiggle.model.json");
    ...
    let result = super::run_survival(SurvivalArgs {
        ...
        exit: "exit".to_string(),
        event: "event".to_string(),
        formula: format!("1 + linkwiggle(degree={degree}, internal_knots=2)"),
        predict_noise: None,
        survival_likelihood: "location-scale".to_string(),
        survival_distribution: "gaussian".to_string(),
        link: None,
    ...
    match result {
        Ok(_) => println!("[2695-WITNESS] degree={degree} FIT OK"),
        Err(e) => println!("[2695-WITNESS] degree={degree} FAILED: {}", &e[..e.len().min(300)]),
    }
    }
}
